...
def get_random_input_tensors(input_shapes):
    ...
    inputs = {
        k: Tensor.randn(*shp, requires_grad=False) * 8
...
def compile(dat, output_fn):
    ...
    for i in range(2):
        tinygrad_out = model_exec(run_onnx, i == 1 and using_graph, **inputs)
    graph.GRAPH = False
    print("kernel count:", len(model_exec.jit_cache))
    assert (
        len(model_exec.jit_cache) <= ALLOWED_KERNEL_COUNT or ALLOWED_KERNEL_COUNT == 0
    ), "too many kernels!"

    # pull out inputs and put them in the jit cache
    input_rawbuffers = {k: inputs[k].lazydata.realized for k in inputs.keys()}
    for (j, i), (idx, _, _) in model_exec.input_replace.items():
        model_exec.jit_cache[j][1][i] = input_rawbuffers[idx]
    ...
    for prg, args in model_exec.jit_cache:
        ...
        cl_cache.append(
            (prg.clprg, [prg.global_size, prg.local_size, *[x._buf for x in args]])
        )
    ...
    print(
        f"buffers to save: {len(t.buffers_to_save)}, inputs: {list(t.inputs.keys())}, outputs: {t.outputs}"
    )
    runtime = t.run()
    print(
        f"network using {used_ops/1e9:.2f} GOPS with runtime {runtime*1e3:.2f} ms that's {used_ops/runtime*1e-9:.2f} GFLOPS"
    )

    # confirm thneed found the right output
    thneed_out = np.empty((t.outputs[0].size // 4,), dtype=np.float32).reshape(
        tinygrad_out.shape
    )
    ...
    if FLOAT16 == 0:
        try:
            ...
            print(
                ...
                torch_out,
                "mse",
                np.sum((thneed_out - torch_out) ** 2),
                "max err",
                np.max(np.abs((thneed_out - torch_out))),
            )
            ...
            t.run()
            old_thneed_out = np.empty(
                (t.outputs[0].size // 4,), dtype=np.float32
            ).reshape(tinygrad_out.shape)
            cl.enqueue_copy(
                CL.cl_queue[0], old_thneed_out, t.outputs[0], is_blocking=True
            )

            # compare thneed (rerun) with torch
            np.testing.assert_allclose(
                new_torch_out, old_thneed_out, atol=1e-4, rtol=1e-2
            )
            ...
            nt.run()
            new_thneed_out = np.empty(
                (nt.outputs[0].size // 4,), dtype=np.float32
            ).reshape(tinygrad_out.shape)
            cl.enqueue_copy(
                CL.cl_queue[0], new_thneed_out, nt.outputs[0], is_blocking=True
            )

            # compare torch to thneed
            np.testing.assert_allclose(
                new_torch_out, new_thneed_out, atol=1e-4, rtol=1e-2
            )
