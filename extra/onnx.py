...
def get_run_onnx(onnx_model):
    def shape_to_tuple(s):
        return tuple(x.dim_value for x in s.dim)
    ...
    def buffer_parse(inp):
        if inp.data_type in (1, 10, 6, 7):
            ...
            if len(inp.float_data) > 0:
                ...
                ret = Tensor(
                    np.array(inp.int64_data, dtype=np.float32).reshape(inp.dims),
                    requires_grad=False,
                )
            elif len(inp.int32_data) > 0:
                ret = Tensor(
                    np.array(inp.int32_data, dtype=np.float32).reshape(inp.dims),
                    requires_grad=False,
                )
            else:
                ret = Tensor(
    ...
    def run_onnx(inputs={}, debug=False):
        ...
        for inp in onnx_model.graph.input:
            ...
            if len(shape) >= 1 and shape[0] == 0:
                shape = tuple([1] + list(shape[1:]))  # 1 batch size
            if inp.name in inputs:
                input_shape = inputs[inp.name].shape
                if input_shape == (0,):
                    raise NotImplementedError(
                        "empty tensors aren't supported in tinygrad"
                    )
                assert (
                    input_shape == shape
        ...
        for num, n in enumerate(onnx_model.graph.node):
            inp = [
                ...
                if x in tensors
                    ...
                    intermediate_tensors[x]
                    if x in intermediate_tensors
                    else (input_tensors[x] if x != str() else None)
                )
                for x in n.input
            ]
            ...
            # one liners
                ...
                ret = inp[0].elu(alpha=opt.get("alpha", 1.0))
            elif n.op_type == "Concat":
                ret = inp[0].cat(*inp[1:], dim=opt["axis"])
            elif n.op_type == "Transpose":
                ret = inp[0].permute(
                    order=opt.get("perm", list(range(len(inp[0].shape))[::-1]))
                )
            elif n.op_type == "Squeeze":
                ret = inp[0].reshape(
                    [s for i, s in enumerate(inp[0].shape) if i not in opt["axes"]]
                )
            elif n.op_type == "Div":
                # in openpilot, due to SHUFFLE_PAD_OPS issues, we are spending an extra kernel
                ret = inp[0].div(inp[1])
            elif n.op_type == "Constant":
                ret = opt["value"] if "value" in opt else opt["value_float"]
            elif n.op_type == "Reshape":
                ret = inp[0].reshape(
                    [
                        int(x) if x != 0 else inp[0].shape[i]
                        for i, x in enumerate(safe_numpy(inp[1]))
                    ]
                )
                ...
                ret = inp[0].reshape(
                    [
                        val
                        for pair in zip(inp[0].shape, [1] * len(scales))
                        for val in pair
                    ]
                )
                ret = ret.expand(
                    [
                        val
                        for pair in zip(inp[0].shape, [int(x) for x in scales])
                        for val in pair
                    ]
                )
                ret = ret.reshape(
                    [x * y for x, y in zip(inp[0].shape, [int(x) for x in scales])]
                )
                ...
                indices = [
                    shape[axis] + int(x) if x < 0 else int(x)
                    for x in safe_numpy(inp[1])
                ]
                args = [
                    [(0, x) if j != axis else (i, i + 1) for j, x in enumerate(shape)]
                    for i in indices
                ]
                ret = (
                    inp[0]
                    .slice(arg=args[0])
                    .cat(*[inp[0].slice(arg=arg) for arg in args[1:]], dim=axis)
                )
                ret = (
                    ret.reshape([s for i, s in enumerate(shape) if i != axis])
                    if len(indices) == 1
                    else ret
                )  # squeeze if needed
                ...
                inp_0 = (
                    inp[0]
                    if isinstance(inp[0], Tensor)
                    else Tensor(np.array(inp[0], dtype=np.float32), requires_grad=False)
                )
                inp_1 = (
                    inp[1]
                    if isinstance(inp[1], Tensor)
                    else Tensor(np.array(inp[1], dtype=np.float32), requires_grad=False)
                )
                if (len(inp_0.shape) != len(inp_1.shape)) and (
                    prod(inp_0.shape) == prod(inp_1.shape)
                ):
                ...
                if "broadcast" in opt:
                    inp_1 = inp_1.reshape(
                        [
                            -1 if i == opt["broadcast"] else 1
                            for i in range(len(inp_0.shape))
                        ]
                    )
                if n.op_type == "Add":
                    ret = inp_0 + inp_1
                ...
                if "split" not in opt:
                    opt["split"] = [
                        int(x) for x in safe_numpy(inp[1])
                    ]  # split can be a tensor
                if "axis" not in opt:
                    opt["axis"] = 0
                i = 0
                ...
                assert (
                    onnx_version >= 10
                ), f"only onnx version >= 10 supported for slice"
                arg = [(0, x) for x in inp[0].shape]
                starts, ends, axes = inp[1:4]
                assert axes.shape == (1,)
                axis, starts, ends = (
                    int(safe_numpy(axes)[0]),
                    int(safe_numpy(starts)[0]),
                    int(safe_numpy(ends)[0]),
                )
                ...
                ret = (inp[0] < -opt["lambd"]) * (inp[0] + bias) + (
                    inp[0] > opt["lambd"]
                ) * (inp[0] - bias)
