    ...
    def buffer_parse(inp):
        if inp.data_type in (1, 10, 6, 7):
            ...
            if len(inp.float_data) > 0:
                ...
                ret = Tensor(
                    np.frombuffer(inp.raw_data, dtype=tensor_dtype_to_np_dtype(inp.data_type)).reshape(inp.dims).astype(
                        np.float32).copy(), requires_grad=False)
        else:
    ...
    def run_onnx(inputs={}, debug=False):
        ...
        for inp in onnx_model.graph.input:
            ...
            if inp.name in inputs:
                ...
                if input_shape == (0,):
                    raise NotImplementedError("empty tensors aren't supported in tinygrad")
        ...
        for num, n in enumerate(onnx_model.graph.node):
            inp = [tensors[x] if x in tensors else (
                intermediate_tensors[x] if x in intermediate_tensors else (input_tensors[x] if x != str() else None))
                   for x in
                   n.input]
            ...
            # one liners
                ...
                ret = inp[0].permute(order=opt.get('perm', list(range(len(inp[0].shape))[::-1])))
                ...
                ret = inp[0].reshape([int(x) if x != 0 else inp[0].shape[i] for i, x in enumerate(safe_numpy(inp[1]))])
                ...
                ret = ret.reshape([s for i, s in enumerate(shape) if i != axis]) if len(
                    indices) == 1 else ret  # squeeze if needed
                ...
                # TODO: is this right?
                if 'broadcast' in opt:
                    inp[1] = inp[1].reshape([-1 if i == opt['broadcast'] else 1 for i in range(len(inp[0].shape))])
                if n.op_type == "Add":
                    ret = inp[0] + inp[1]
                if n.op_type == "Sub":
                    ret = inp[0] - inp[1]
