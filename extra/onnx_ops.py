...
def LayerNormalization(x: Tensor, scale, bias, axis=-1, epsilon=1e-05, stash_type=1):
    ...
    return x.layernorm(axis, epsilon).mul(scale).add(bias), mean, (x.sub(mean)).pow(2).mean(axis=axis,
                                                                                            keepdim=True).add(
        epsilon).sqrt().reciprocal()


def GroupNormalization(x: Tensor, scale: Tensor, bias: Tensor, num_groups, epsilon=1e-05):
    return x.reshape(x.shape[0], num_groups, -1).layernorm(axis=-1, eps=epsilon).mul(scale.unsqueeze(-1)).add(
        bias.unsqueeze(-1)).reshape(x.shape)
...
def AveragePool(X, kernel_shape, auto_pad="NOTSET", ceil_mode=0, count_include_pad=0, dilations=1, pads=None,
                strides=1):
...
def MaxPool(X, kernel_shape, auto_pad="NOTSET", ceil_mode=0, dilations=1, pads=None, storage_order=0, strides=1):
    ...
    return _padding(X, pads, auto_pad, constant_value=-np.inf, axes=tuple(range(len(X.shape)))[-2:]).max_pool2d(
        kernel_shape, stride=strides)
...
def Conv(X, W, B=None, auto_pad="NOTSET", dilations=1, group=1, kernel_shape=None, pads=None, strides=1):
    return X.conv2d(W, B, stride=strides, groups=group, dilation=dilations,
                    padding=(pads[1], pads[3], pads[0], pads[2]) if pads is not None else 0)
...
def Dropout(data, ratio=0.5, training_mode=False, seed=None):
    ...
    if not training_mode:
        return data, Tensor.ones(*data.shape)  # if mask is requested as output it will contain all ones.
    if seed is not None:
...
def Shape(data, end=None, start=0):
    return list(data.shape)[start:end]


def Size(data):
    return prod(data.shape)
...
def Expand(input, shape):
    ...
    x_shape, y_shape = [([1] * (max(len(x_shape), len(y_shape)) - len(t_shape)) + list(t_shape)) for t_shape in
                        [x_shape, y_shape]]
...
def LRN(input, size, alpha=1e-4, beta=0.75, bias=1.0):
    ...
    return input / input.mul(input).reshape(bs, 1, c, iy * ix).pad2d((0, 0, (size - 1) // 2, size // 2)).avg_pool2d(
        (size, 1), 1).reshape(bs, c, iy, ix).mul(alpha).add(bias).pow(beta)
...
def HardSigmoid(input, alpha=0.2, beta=0.5):
...
def ThresholdedRelu(X, alpha=1.0):
    return (X - alpha).relu() + (X - alpha).relu().sign() * alpha


def Softmax_1(input, axis=1):
    return input.softmax(axis)


def Softmax_13(input, axis=-1):
    return input.softmax(axis)
...
def LogSoftmax(input, axis=-1):
    return input.log_softmax(axis)


def Clip(input, min=-3.4e38, max=3.4e38):
    return input.clip(min, max)


def Less(x, y):
    return (x < y).numpy().astype(bool)
...
def Min(*data_0):
    return -functools.reduce(Tensor.maximum, [-x for x in data_0])
...
def _axes(axes, noop_with_empty_axes):
    return [int(x) for x in safe_numpy(axes)] if axes is not None else ([] if noop_with_empty_axes else None)


# ReduceProd would require a new llop
def ReduceMax(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.max(_axes(axes, noop_with_empty_axes), keepdim=keepdims)


def ReduceMin(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.min(_axes(axes, noop_with_empty_axes), keepdim=keepdims)


def ReduceSum(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.sum(_axes(axes, noop_with_empty_axes), keepdim=keepdims)


def ReduceMean(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.mean(_axes(axes, noop_with_empty_axes), keepdim=keepdims)


def ReduceSumSquare(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.square().sum(_axes(axes, noop_with_empty_axes), keepdim=keepdims)


def ReduceL1(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.abs().sum(_axes(axes, noop_with_empty_axes), keepdim=keepdims)


def ReduceL2(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.square().sum(_axes(axes, noop_with_empty_axes), keepdim=keepdims).sqrt()


def ReduceLogSum(data, axes=None, keepdims=1, noop_with_empty_axes=0):
    return data.sum(_axes(axes, noop_with_empty_axes), keepdim=keepdims).log()


def ReduceLogSumExp(data, axes=None, keepdims=1, noop_with_empty_axes=0):
...
def Range(start, limit, delta):
    return Tensor.arange(safe_numpy(limit)[0], safe_numpy(start)[0], safe_numpy(delta)[0])
