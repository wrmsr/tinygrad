...
def Gemm(A, B, C=None, alpha=1.0, beta=1.0, transA=0, transB=0):
    ret = alpha * (
        (A.transpose() if transA == 1 else A) @ (B.transpose() if transB == 1 else B)
    )
    if C is not None:
        ret += beta * C
    return ret
...
def _padding(X, pads=None, auto_pad="NOTSET", axes=None, constant_value=0.0):
    ...
    constant_padder = Tensor(
        np.pad(np.zeros(X.shape), np_pads, constant_values=constant_value),
        dtype=X.dtype,
    )
...
def Pad(
    ...
    seq_pads = (
        ...
        else pads.numpy().astype(np.int32).tolist()
    )
...
def AveragePool(
    ...
    padding_included = _padding(X, pads, auto_pad, axes=pixel_axes).avg_pool2d(
    ...
    if count_include_pad:
        ...
        div = _padding(
            Tensor.ones(*X.shape), pads, auto_pad, axes=pixel_axes
...
def MaxPool(
    ...
    return _padding(
        X, pads, auto_pad, constant_value=-np.inf, axes=tuple(range(len(X.shape)))[-2:]
    ).max_pool2d(kernel_shape, stride=strides, dilation=dilations)
...
def Dropout(data, ratio=0.5, training_mode=False, seed=None):
    ...
    _mask: np.ndarray = np.asarray(
        Tensor._rng.binomial(1, 1.0 - ratio, size=data.shape), dtype=data.dtype
    )
...
def Range(start, limit, delta):
    return Tensor.arange(
        safe_numpy(limit)[0], safe_numpy(start)[0], safe_numpy(delta)[0]
    )
...
def And(x: Tensor, y: Tensor):
    return Where((x == y), x, Tensor.zeros(*x.shape)).cast(dtypes.bool)


def Or(x: Tensor, y: Tensor):
    return Where((x == y), x, Tensor.ones(*x.shape)).cast(dtypes.bool)


def Xor(x: Tensor, y: Tensor):
    return Where((x == y), Tensor.zeros(*x.shape), Tensor.ones(*x.shape)).cast(
        dtypes.bool
    )


def Not(x: Tensor):
    return Where((x == 1), Tensor.zeros(*x.shape), Tensor.ones(*x.shape)).cast(
        dtypes.bool
    )
