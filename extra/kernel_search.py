...
def get_interventions(k, winning_interventions=[]):
    ...
    p1, p2, p3, p4, p5 = [], [], [], [], []
    p1 = [
        (Interventions.SWAP, x)
        for x in itertools.combinations(range(k.first_reduce), 2)
    ]
    p2 = [
        (Interventions.SWAP, x)
        for x in itertools.combinations(
            range(k.first_reduce + len(k.group_for_reduce), k.shape_len), 2
        )
    ]
    p3 = (
        [(Interventions.UPCAST, None)]
        if max(st.shape[-1] for st in k.sts) <= 32
        else []
    )
    for up_axis in range(k.shape_len):
        if up_axis >= k.first_reduce and up_axis < (
            k.first_reduce + len(k.group_for_reduce)
        ):
            continue
        ...
        for amount in sorted(list(set([2, 4, 8, max_up]))):
            if amount >= 32:
                continue
            if not all(
                st.shape[up_axis] == 1 or st.shape[up_axis] % amount == 0
                for st in k.sts
            ):
                continue
    ...
    if typ == Interventions.SWAP:
        ...
        if dat is not None:
            ...
            k.reshape_and_permute(
                ...
                + ([x[up_axis] // amount, amount] if x[up_axis] > 1 else [1, 1])
                + list(x[up_axis + 1 :]),
        ...
        k.reshape_and_permute(
            ...
            + [up_axis + 1]
            + [i for i in range(k.first_reduce, k.shape_len + 1) if i != up_axis + 1],
        )
...
def apply_optimization(k, ast, max_interventions=1, cache=True):
    ...
    if k.key not in intervention_cache or cache == False:
        ...
        for i in range(
            max_interventions
        ):  # NOTE: multiple interventions is breaking the ASTs
...
def randomize_buffers(ast):
    ...
    for b in bufs:
        randomness = np.random.default_rng().standard_normal(
            size=b._base_shape, dtype=np.float32
        )
...
if __name__ == "__main__":
    ...
    if getenv("OP", 0) == 1:
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(1, 64, 128, 8, 4, 3, 3, 3, 4),
                views=[
                    View((1, 130, 258, 1, 12), (393216, 3072, 12, 12, 1), -3084),
                    ZeroView(
                        (1, 128, 256, 1, 12),
                        ((0, 1), (-1, 129), (-1, 257), (0, 1), (0, 12)),
                    ),
                    View(
                        (1, 64, 128, 8, 4, 3, 3, 3, 4),
                        (0, 6192, 24, 0, 0, 3096, 12, 4, 1),
                        0,
                    ),
                ],
            ),
            hostbuf=GPUBuffer(shape=(128, 768, 4), force_create=True),
        )
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                shape=(1, 64, 128, 8, 4, 3, 3, 3, 4),
        ...
        buf3 = GPUBuffer(
            ...
            hostbuf=GPUBuffer(shape=(1,), backing=np.array([1.0], dtype=np.float32)),
        )
        buf4 = GPUBuffer(
            shape=ShapeTracker(
                shape=(1, 64, 128, 8, 4, 1, 1, 1, 1),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(1, 64, 128, 8, 4, 1, 1, 3, 3),
                views=[
                    View((1, 66, 130, 32, 1), (262144, 4096, 32, 1, 1), -4128),
                    ZeroView(
                        (1, 64, 128, 32, 1),
                        ((0, 1), (-1, 65), (-1, 129), (0, 32), (0, 1)),
                    ),
                    View(
                        (1, 64, 128, 8, 4, 1, 1, 3, 3),
                        (266240, 4160, 32, 4, 1, 12480, 12480, 4160, 32),
                        0,
                    ),
                ],
            ),
        ...
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                ...
                views=[
                    View(
                        (1, 64, 128, 8, 4, 1, 1, 3, 3), (0, 0, 0, 36, 1, 0, 0, 12, 4), 0
                    )
                ],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(1, 64, 128, 4, 4, 1, 1, 8, 4),
                views=[
                    View(
                        (1, 64, 128, 4, 4, 1, 1, 8, 4),
                        (0, 4096, 32, 0, 0, 0, 0, 4, 1),
                        0,
                    )
                ],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(32, 8, 112, 112),
                views=[View((32, 8, 112, 112), (12544, 401408, 112, 1), 0)],
            ),
        ...
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                shape=(32, 1, 1, 1), views=[View((32, 1, 1, 1), (0, 0, 0, 0), 0)]
            ),
            hostbuf=GPUBuffer(
                shape=(1,), backing=np.array([9.964923e-06], dtype=np.float32)
            ),
        )
        op1 = LazyOp(
            BinaryOps.MUL,
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(64, 1, 128, 3, 3, 512, 32, 32),
                views=[
                    View((64, 512, 34, 34), (1024, 65536, 32, 1), -33),
                    ZeroView(
                        (64, 512, 32, 32), ((0, 64), (0, 512), (-1, 33), (-1, 33))
                    ),
                    View(
                        (64, 1, 128, 3, 3, 512, 32, 32),
                        (591872, 591872, 0, 34, 1, 1156, 34, 1),
                        0,
                    ),
                ],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(8, 1, 32, 112, 112, 3, 3, 3),
                views=[
                    View((8, 3, 225, 225), (150528, 50176, 224, 1), 0),
                    ZeroView((8, 3, 224, 224), ((0, 8), (0, 3), (0, 225), (0, 225))),
                    View(
                        (8, 1, 32, 112, 112, 3, 3, 3),
                        (151875, 151875, 0, 450, 2, 50625, 225, 1),
                        0,
                    ),
                ],
            ),
        ...
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                shape=(8, 1, 32, 112, 112, 3, 3, 3),
                views=[
                    View((8, 1, 32, 112, 112, 3, 3, 3), (0, 0, 27, 0, 0, 9, 3, 1), 0)
                ],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(64, 512, 32, 32),
                views=[View((64, 512, 32, 32), (524288, 1024, 32, 1), 0)],
            ),
        ...
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                shape=(64, 1, 1, 1), views=[View((64, 1, 1, 1), (0, 0, 0, 0), 0)]
            ),
            hostbuf=GPUBuffer(
                shape=(1,), backing=np.array([1.9073486e-06], dtype=np.float32)
            ),
        )
        op1 = LazyOp(
            BinaryOps.MUL,
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(1, 1, N, N, 1, 1, 1, N),
                views=[
                    View((1, N, N, 1), (0, 1, N, 0), 0),
                    View((1, 1, N, N, 1, 1, 1, N), (0, 0, 0, 1, 0, 0, 0, N), 0),
                ],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(32, 1, 32, 32, 32, 64, 3, 3),
                views=[
                    View(
                        (32, 1, 32, 32, 32, 64, 3, 3),
                        (73984, 73984, 0, 34, 1, 1156, 34, 1),
                        0,
                    )
                ],
            ),
        ...
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                shape=(32, 1, 32, 32, 32, 64, 3, 3),
                views=[
                    View((32, 1, 32, 32, 32, 64, 3, 3), (0, 0, 576, 0, 0, 9, 3, 1), 0)
                ],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(64, 1, 1, 1), views=[View((64, 1, 1, 1), (1, 0, 0, 0), 0)]
            ),
        ...
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                shape=(64, 5, 32, 32),
                views=[View((64, 5, 32, 32), (5120, 1024, 32, 1), 0)],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(5, 1, 128, 16, 16, 128, 3, 3),
                views=[
                    View((5, 128, 18, 18), (32768, 256, 16, 1), -17),
                    ZeroView((5, 128, 16, 16), ((0, 5), (0, 128), (-1, 17), (-1, 17))),
                    View(
                        (5, 1, 128, 16, 16, 128, 3, 3),
                        (41472, 41472, 0, 18, 1, 324, 18, 1),
                        0,
                    ),
                ],
            ),
        ...
        buf0 = GPUBuffer(
            shape=ShapeTracker(
                shape=(3, 1, 32, 3, 3, 32, 112, 112),
                views=[
                    View((3, 32, 225, 225), (50176, 150528, 224, 1), 0),
                    ZeroView((3, 32, 224, 224), ((0, 3), (0, 32), (0, 225), (0, 225))),
                    View(
                        (3, 1, 32, 3, 3, 32, 112, 112),
                        (1620000, 1620000, 0, 225, 1, 50625, 450, 2),
                        0,
                    ),
                ],
            ),
        ...
        buf1 = GPUBuffer(
            shape=ShapeTracker(
                ...
                views=[
                    View(
                        (3, 1, 32, 3, 3, 32, 112, 112),
                        (0, 12845056, 401408, 0, 0, 12544, 112, 1),
                        0,
                    )
                ],
            ),
