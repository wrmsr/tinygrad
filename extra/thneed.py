...
class Thneed:
    ...
    def load(self, input_fn):
        ...
        for o in jdat["objects"]:
            ...
            if o["arg_type"] == "image2d_t" or o["arg_type"] == "image1d_t":
                ...
                if "data" in o:
                    buf = cl.Buffer(
                        CL.cl_ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=o["data"]
                    )
                else:
                    # zero out buffers
                    buf = cl.Buffer(
    ...
    def save(self, output_fn):
        ...
        for prg, args in self.cl_cache:
            ...
            targs, args_size = [], []
            argdtypes = (
                prg.argdtypes if prg.argdtypes is not None else [None] * (len(args) - 2)
            )
            for a, d in zip(args[2:], argdtypes):
                    ...
                    if ptr not in saved_objs:
                        if isinstance(a, cl.Buffer):
                            ...
                            if needs_load:
                                ...
                                cl.enqueue_copy(
                                    CL.cl_queue[0], data, a, is_blocking=True
                                )
              ...
              """,
                            ...
                            if needs_load:
                                ...
                                cl.enqueue_copy(
                                    CL.cl_queue[0], data, buf, is_blocking=True
                                )
                                if FLOAT16:
    ...
    def run(self):
        ...
        print(
            f"submit in {(mt - st) * 1000.0:.2f} ms, total runtime is {et * 1000.0:.2f} ms"
        )
