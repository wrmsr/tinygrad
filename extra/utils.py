...
def my_unpickle(fb0):
    ...
    def _rebuild_tensor_v2(
        ...
        if storage_type not in [np.float16, np.float32]:
            if DEBUG:
                print(f"unsupported type {storage_type} on {obj_key} with shape {size}")
            ret = None
        else:
            ret = Tensor(
                LazyNumpyArray(
                    lambda lst: np.zeros(lst.shape, dtype=lst.dtype),
                    tuple(size),
                    storage_type,
                )
            )
        key_prelookup[obj_key].append(
            (storage_type, obj_size, ret, size, stride, storage_offset)
        )
        return ret
...
def load_single_weight(
    t: Tensor, myfile, shape, strides, dtype, storage_offset, mmap_allowed=False
):
    ...
    if any(
        s != 1 and st1 != st2
        for s, st1, st2 in zip(shape, strides_for_shape(shape), strides)
    ):
        # slow path
        buffer_size = sum(
            strides[i] * t.dtype.itemsize * (shape[i] - 1) for i in range(len(shape))
        )
    ...
    if t.device in ["METAL", "CLANG", "LLVM"]:
        del t.lazydata.op
        t.lazydata.realized = Device[t.lazydata.device].buffer(
            prod(t.shape), dtype=t.dtype
        )
        ...
        def _mmap(lna):
            ...
            return np.memmap(
                myfile._fileobj._file,
                dtype=lna.dtype,
                mode="r",
                offset=myfile._orig_compress_start + bytes_offset,
                shape=lna.shape,
            )
        ...
        if mmap_allowed and not OSX and t.device in ["GPU", "CUDA"]:
            t.lazydata.op.arg.fxn = _mmap
        else:
        ...
        if load_weights:
            ...
            def load_weight(k, vv):
                with myzip.open(f"{base_name}/data/{k}") as myfile:
                    for v in vv:
                        load_single_weight(
                            v[2], myfile, v[3], v[4], v[0], v[5], mmap_allowed=True
                        )
            ...
            if multithreaded:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                    futures = {
                        executor.submit(load_weight, k, v): k for k, v in ret[1].items()
                    }
                    for future in (
                        t := tqdm(
                        ...
                        k = futures[future]
                        t.set_description(
                            f"loading {k} ram used: {GlobalCounters.mem_used/1e9:5.2f} GB"
                        )
            else:
