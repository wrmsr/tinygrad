...
class _CL:
    def __init__(self):
        platforms: List[List[cl.Device]] = [
            y
            for y in (
                [
                    x.get_devices(device_type=cl.device_type.GPU)
                    for x in cl.get_platforms()
                ]
                + [
                    x.get_devices(device_type=cl.device_type.CPU)
                    for x in cl.get_platforms()
                ]
            )
            if len(y)
        ...
        self.cl_ctx: cl.Context = cl.Context(
            devices=[
                x
                for x in platforms[getenv("CL_PLATFORM", 0)]
                if x.name not in getenv("CL_EXCLUDE", "").split(",")
            ]
        )
        self.cl_queue: List[cl.CommandQueue] = [
            ...
            for device in self.cl_ctx.devices
        ]
...
class CLProgram:
    ...
    def __call__(self, global_size, local_size, *bufs, wait=False) -> Optional[float]:
        ...
        e = self.clprg(
            CL.cl_queue[cl_bufs[0].device], global_size, local_size, *cl_bufs
        )
        if wait:
            e.wait()
            try:
