...
OSX_TIMING_RATIO = (
    125 / 3) if OSX else 1.0  # see test/external_osx_profiling.py to determine this ratio. it's in like GPU clocks or something
...
class _CL:
    def __init__(self):
        platforms: List[List[cl.Device]] = [y for y in (
            [x.get_devices(device_type=cl.device_type.GPU) for x in cl.get_platforms()] + [
            x.get_devices(device_type=cl.device_type.CPU) for x in cl.get_platforms()]) if len(y)]
        if DEBUG >= 1:
            print(f"using {platforms[getenv('CL_PLATFORM', 0)]}")
        self.cl_ctx: cl.Context = cl.Context(devices=platforms[getenv('CL_PLATFORM', 0)])
        self.cl_queue: List[cl.CommandQueue] = [
            cl.CommandQueue(self.cl_ctx, device=device, properties=cl.command_queue_properties.PROFILING_ENABLE) for
            device in
            self.cl_ctx.devices]
    ...
    def __init__(self, size, dtype, device=0):
        if isinstance(dtype, ImageDType):
            fmt = cl.ImageFormat(cl.channel_order.RGBA,
                                 {2: cl.channel_type.HALF_FLOAT, 4: cl.channel_type.FLOAT}[dtype.itemsize])
...
class CLProgram:
    def __init__(self, name: str, prg: str, binary=False, argdtypes=None, options=None):
        ...
        self.clprogram = cl.Program(CL.cl_ctx, CL.cl_ctx.devices, [prg] * len(CL.cl_ctx.devices)) \
            if binary else cl.Program(CL.cl_ctx, prg)  # type: ignore
        try:
        ...
        if DEBUG >= 5 and not OSX:
            if 'Adreno' in CL.cl_ctx.devices[0].name:
                ...
                asm = early_exec((
                    [
                        # pathlib.Path(__file__).parent.parent.parent / "extra/rocm/build/llvm-project/bin/llvm-objdump",
                        ROCM_LLVM_PATH / "llvm-objdump",
                        '-d',
                        '-',
                    ],
                    self.binary(),
    ...
    def binary(self):
        return self.clprogram.get_info(cl.program_info.BINARIES)[0]

    @staticmethod
    def max_work_group_size():
        return CL.cl_ctx.devices[0].max_work_group_size
