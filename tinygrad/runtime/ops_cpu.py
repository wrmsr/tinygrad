...
def shape_to_axis(old_shape: ta.Tuple[int, ...], new_shape: ta.Tuple[int, ...]) -> ta.Tuple[int, ...]:
    assert len(old_shape) == len(new_shape), "reduce shapes must have same dimensions"
    return tuple(i for i, (a, b) in enumerate(zip(old_shape, new_shape)) if a != b)


base_fxn_for_op: ta.Dict[Op, ta.Callable] = {
    BinaryOps.ADD: operator.add,
    BinaryOps.SUB: operator.sub,
    BinaryOps.MUL: operator.mul,
    BinaryOps.DIV: operator.truediv,
    BinaryOps.POW: operator.pow,
...
def einsum_mulacc(einsum, get_strides, expand):
    ...
    def axes_slice(strides):
        return (
            [i for i in range(len(strides)) if strides[i] != 0],
            tuple(slice(None) if strides[i] != 0 else 0 for i in range(len(strides))),
        )
    ...
    def mulacc(a, b, new_shape):
        ...
        out = [
            ...
            if a.shape[i] == new_shape[i]
            and (i in a_axes or i in b_axes)
        ]
