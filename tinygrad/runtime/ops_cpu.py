...
base_fxn_for_op: Dict[Op, Callable] = {
    ...
    ReduceOps.SUM: lambda x, new_shape: x.sum(shape_to_axis(x.shape, new_shape), keepdims=True) if tuple(
        x.shape) != tuple(new_shape) else x[:],
    ReduceOps.MAX: lambda x, new_shape: (x.amax if hasattr(x, 'amax') else x.max)(shape_to_axis(x.shape, new_shape),
                                                                                  keepdims=True) if tuple(
        x.shape) != tuple(new_shape) else x[:],
...
def einsum_mulacc(einsum, get_strides, expand):
    ...
    def axes_slice(strides): return [i for i in range(len(strides)) if strides[i] != 0], tuple(
        slice(None) if strides[i] != 0 else 0 for i in range(len(strides)))
    ...
    def mulacc(a, b, new_shape):
        ...
        return expand(ret.reshape([(1 if i not in a_axes and i not in b_axes else s) for i, s in enumerate(new_shape)]),
                      new_shape)
...
numpy_fxn_for_op: Dict[Op, Callable] = {**base_fxn_for_op, **{
    ...
    UnaryOps.CAST: lambda x, y: x.astype(y.np), UnaryOps.SIN: np.sin,
