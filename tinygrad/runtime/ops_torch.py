...
torch_fxn_for_op: Dict[Op, Callable] = {
    ...
    **{
        ...
        MovementOps.STRIDE: (
            lambda x, arg: x[tuple(slice(None, None, abs(i)) for i in arg)].flip([i for i, a in enumerate(arg) if a < 0])
        ),
...
class RawTorchBuffer(RawBuffer):
    def __init__(self, buf: torch.Tensor):
        super().__init__(prod(buf.shape), type_map[buf.dtype], buf)

    @classmethod
    def fromCPU(cls, x):
        return cls(torch.from_numpy(x).requires_grad_(False).to(device))
