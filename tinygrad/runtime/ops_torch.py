...
torch_fxn_for_op: Dict[Op, Callable] = {**base_fxn_for_op, **{
    ...
    UnaryOps.CAST: lambda x, y: x.type(next(k for k, v in type_map.items() if v == y)),
    BinaryOps.MAX: torch.maximum, BinaryOps.CMPEQ: lambda x, y: (x == y).float(),
    MovementOps.PAD: lambda x, padding: torch.nn.functional.pad(x,
                                                                [item for sublist in padding[::-1] for item in
                                                                 sublist]),
