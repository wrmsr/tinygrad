...
torch_fxn_for_op: Dict[Op, Callable] = {**base_fxn_for_op, **{
    ...
    UnaryOps.CAST: lambda x, y: x.type(next(k for k, v in type_map.items() if v == y)), UnaryOps.SIN: torch.sin,
    BinaryOps.MAX: torch.maximum, BinaryOps.CMPEQ: lambda x, y: (x == y).float(),
    MovementOps.PAD: lambda x, padding: torch.nn.functional.pad(x, [item for sublist in padding[::-1] for item in
                                                                    sublist]),
    FusedOps.MULACC: einsum_mulacc(
    ...
    MovementOps.STRIDE: lambda x, arg: x[tuple(slice(None, None, abs(i)) for i in arg)].flip(
        [i for i, a in enumerate(arg) if a < 0]),
