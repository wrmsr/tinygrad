...
class RawBuffer:  # pylint: disable=abstract-method
    def __init__(self, size: int, dtype: DType, buf: ta.Any = None) -> None:
        super().__init__()
        self.size: int = size
    ...
    def __repr__(self):
        return f"buffer<{self.size}, {self.dtype}>"

    # NOTE: this interface allows for 0 copy
    @classmethod
    def fromCPU(cls: ta.Type[_T], x: np.ndarray) -> _T:
...
class RawMallocBuffer(RawBufferMapped):
    def __init__(self, size, dtype: DType):
        super().__init__(
            size,
            dtype,
            ({dtypes.float32: ctypes.c_float, dtypes.float16: ctypes.c_int16}[dtype] * size)(),
        )
