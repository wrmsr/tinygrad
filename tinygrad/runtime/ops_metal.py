...
class RawMetalBuffer(RawBufferMapped):
    def __init__(self, size: int, dtype: DType): super().__init__(size, dtype,
                                                                  METAL.device.newBufferWithLength_options_(
                                                                      size * dtype.itemsize,
...
class MetalProgram:
    def __init__(self, name: str, prg: str):
        ...
        if DEBUG >= 5:
            arc = unwrap(
                METAL.device.newBinaryArchiveWithDescriptor_error_(Metal.MTLBinaryArchiveDescriptor.alloc().init(),
                                                                   None))
            ...
            os.system(
                f"cd {pathlib.Path(__file__).parent.parent.parent}/disassemblers/applegpu && python3 compiler_explorer.py /tmp/shader.bin")
    ...
    def __call__(self, global_size, local_size, *bufs, wait=False):
        ...
        assert prod(
            local_size) <= self.pipeline_state.maxTotalThreadsPerThreadgroup(), f"local size {local_size} bigger than {self.pipeline_state.maxTotalThreadsPerThreadgroup()} with exec width {self.pipeline_state.threadExecutionWidth()} memory length {self.pipeline_state.staticThreadgroupMemoryLength()}"
