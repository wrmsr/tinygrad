...
def to_shape_strides(shape: Tuple[int, ...], strides: Tuple[int, ...]) -> List[Tuple[int, int]]:
    ...
    for i in range(1, len(shape)):
        if (strides[i] != 0 and ret[-1][1] == shape[i] * strides[i]) or ret[-1][0] == 1 or (
            strides[i] == 0 and ret[-1][1] == 0):
...
class View:
    ...
    def expr_idxs(self, idxs):
        ...
        return Variable.sum(
            [Variable.num(self.offset)] + [idx * st for idx, sh, st in zip(idxs, self.shape, self.strides) if
                                           sh != 1 and st != 0])
...
class ShapeTracker:
    ...
    def __unsafe_resize(self, arg: Tuple[Tuple[int, int], ...], mask=None):
        ...
        if self.views[-1].mask:
            # move the old mask
            nmask = tuple(
                (max(mx - ax, 0), min(my - ax, ay - ax)) for (mx, my), (ax, ay) in zip(self.views[-1].mask, arg))
            # merge the masks if we have two
            mask = tuple((max(mx1, mx2), min(my1, my2)) for (mx1, my1), (mx2, my2) in
                         zip(nmask, mask)) if mask is not None else nmask
        self.views[-1] = View(tuple(y - x for x, y in arg), self.views[-1].strides, self.views[-1].offset + offset,
                              mask)
    ...
    def reshape(self, new_shape: Tuple[int, ...]):
        ...
        assert all(isinstance(x, int) and x > 0 for x in
                   new_shape), f"shape must be ints and can't contain 0 or negative numbers {new_shape}"
    ...
    def permute(self, axis: Tuple[int, ...]):
        assert all(isinstance(x, int) and x >= 0 and x < len(self.shape) for x in
                   axis), f"invalid permute {axis} for {self.shape}"
        assert len(set(axis)) == len(axis) and len(axis) == len(self.shape), f"can't permute {self.shape} with {axis}"
        self.views[-1] = View(tuple(self.shape[a] for a in axis), tuple(self.views[-1].strides[a] for a in axis),
                              self.views[-1].offset,
                              tuple(self.views[-1].mask[a] for a in axis) if self.views[-1].mask is not None else None)
    ...
    def stride(self, mul: Tuple[int, ...]):
        ...
        mask = tuple(
            ...
            for (mx, my), s, m in zip(self.views[-1].mask, self.shape, mul)) if self.views[
                                                                                    -1].mask is not None else None
...
dispatch: Dict[MovementOps, Callable] = {MovementOps.RESHAPE: ShapeTracker.reshape,
                                         MovementOps.EXPAND: ShapeTracker.expand, MovementOps.PAD: ShapeTracker.pad,
...
def get_contraction(old_shape: Tuple[int, ...], new_shape: Tuple[int, ...]) -> Optional[List[List[int]]]:
    ...
    while old_shape_i < len(old_shape):
        ...
        if new_shape[i] == 1 and old_shape[old_shape_i] != 1:
            ...
            if new_shape[i] % old_shape[old_shape_i] != 0 or prod([old_shape[x] for x in axis_groups[i]]) * old_shape[
                old_shape_i] > new_shape[i]:
                return None
