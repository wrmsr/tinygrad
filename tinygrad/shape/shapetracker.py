...
def check_no_mul(test, var):
    ...
    if isinstance(test, SumNode):
        return any(check_no_mul(x, var) for x in test.nodes)  # in a sum is okay
    if isinstance(test, ModNode) and test.b % 4 == 0:
        return check_no_mul(test.a, var)  # removing a mod is okay
    return False
...
def to_shape_strides(
    ...
    for i in range(1, len(shape)):
        if (
            (strides[i] != 0 and ret[-1][1] == shape[i] * strides[i])
            or ret[-1][0] == 1
            or (strides[i] == 0 and ret[-1][1] == 0)
        ):
...
class View:
    def __init__(
        ...
        self.shape, self.strides, self.offset = (
            shape,
            tuple(stride if shp != 1 else 0 for stride, shp in zip(strides, shape)),
            offset,
        )
        self.mask = mask
        ...
        self.contiguous: bool = (
            self.offset == 0
            and is_contiguous(self.shape, self.strides)
            and mask is None
        )

    def __repr__(self):
        return f"View({self.shape}, {self.strides}, {self.offset}, {self.mask})"
    ...
    def expr_node(self, idx=None) -> Node:
        if idx is None:
            idx = Variable("idx", 0, prod(self.shape))
    ...
    def expr_idxs(self, idxs):
        assert len(idxs) == len(
            self.shape
        ), f"need an idx for all dimensions {idxs} vs {self.shape}"
        return Variable.sum(
            [Variable.num(self.offset)]
            + [
                idx * st
                for idx, sh, st in zip(idxs, self.shape, self.strides)
                if sh != 1 and st != 0
            ]
        )
...
def merge_views(vm2: View, vm1: View) -> Optional[View]:
    ...
    for s, st in zip(vm1.shape, vm1.strides):
        this_dim = View(vm2.shape, vm2.strides).expr_node(
            Variable("idx", 0, s - 1) * st
        )
        if s == 1:
            ...
            if DEBUG >= 4:
                print("can't simplify", s, this_dim.render())
            break
    return (
        View(vm1.shape, tuple(new_strides), new_offset.b, vm1.mask)
...
class ShapeTracker:
    ...
    def unit_stride_axes(self) -> List[int]:
        ...
        for j, s in list(enumerate(self.shape))[::-1]:
            ...
            acc *= s
            if check_no_mul(this_dim[0], var):
                ret.append(j)
        return ret
    ...
    def expr_node(self, idx="idx"):
        ...
        return self._expr_idx(
            self.views[-1].expr_node(idx), self.views[-1].expr_node_mask(idx)
        )
    ...
    def pad(self, arg: Tuple[Tuple[int, int], ...]):
        ...
        self.__unsafe_resize(
            zvarg, mask=tuple((b, s + b) for s, (b, _) in zip(self.shape, arg))
        )
    ...
    def expand(self, new_shape: Tuple[int, ...]):
        ...
        self.views[-1] = View(
            new_shape, self.views[-1].strides, self.views[-1].offset, mask
        )
    ...
    def reshape(self, new_shape: Tuple[int, ...]):
        if self.shape == new_shape:
            return self
        assert all(
            isinstance(x, int) and x > 0 for x in new_shape
        ...
        if tuple(x for x in self.shape if x != 1) == tuple(
            x for x in new_shape if x != 1
        ):
            old_strides = [
                y for x, y in zip(self.shape, self.views[-1].strides) if x != 1
            ]
            new_strides_tuple = tuple(
                0 if x == 1 else old_strides.pop(0) for x in new_shape
            )
            new_mask_tuple = None
            if self.views[-1].mask:
                if any(
                    y != (0, 1)
                    for x, y in zip(self.shape, self.views[-1].mask)
                    if x == 1
                ):
                    # mask it all out!
                    ...
                    old_mask = [
                        y for x, y in zip(self.shape, self.views[-1].mask) if x != 1
                    ]
                    new_mask_tuple = tuple(
                        (0, 1) if x == 1 else old_mask.pop(0) for x in new_shape
                    )
            self.views[-1] = View(
                new_shape, new_strides_tuple, self.views[-1].offset, new_mask_tuple
            )
            return self
        ...
        if self.contiguous:
            ...
            if (merged_view := merge_views(self.views[-1], view)) is not None:
                self.views[-1] = merged_view
            else:
                if DEBUG >= 4:
                    print(
                        f"WARNING: creating new view with reshape {self} -> {new_shape}"
                    )
    ...
    def permute(self, axis: Tuple[int, ...]):
        ...
        self.views[-1] = View(
            ...
            tuple(self.views[-1].strides[a] for a in axis),
            self.views[-1].offset,
            tuple(self.views[-1].mask[a] for a in axis)
    ...
    def movement_op(
        self, op, arg: Union[Tuple[int, ...], Tuple[Tuple[int, int], ...]]
    ) -> ShapeTracker:
        assert isinstance(arg, tuple) and (
            len(arg) == len(self.shape) or op == MovementOps.RESHAPE
...
dispatch: Dict[MovementOps, Callable] = {
    ...
    MovementOps.SHRINK: ShapeTracker.shrink,
    MovementOps.PERMUTE: ShapeTracker.permute,
    MovementOps.STRIDE: ShapeTracker.stride,
}
...
def get_contraction(
    ...
    while old_shape_i < len(old_shape):
        ...
        if new_shape[i] == 1 and old_shape[old_shape_i] != 1:
            ...
            if (
                new_shape[i] % old_shape[old_shape_i] != 0
                or prod([old_shape[x] for x in axis_groups[i]]) * old_shape[old_shape_i]
                > new_shape[i]
            ):
                return None
