...
def to_shape_strides(shape: Tuple[int, ...], strides: Tuple[int, ...]) -> List[Tuple[int, int]]:
    ...
    for i in range(1, len(shape)):
        if (
            ...
            or ret[-1][0] == 1 or (strides[i] == 0 and ret[-1][1] == 0)
        ):
...
class View:
    ...
    def expr_idxs(self, idxs):
        ...
        return Variable.sum(
            ...
            [idx * st for idx, sh, st in zip(idxs, self.shape, self.strides) if sh != 1 and st != 0]
        )
...
class ShapeTracker:
    ...
    def reshape(self, new_shape: Tuple[int, ...]):
        if self.shape == new_shape:
            return self
        assert all(
            isinstance(x, int) and x != 0 for x in new_shape), f"shape must be ints and can't contain 0 {new_shape}"
    ...
    while old_shape_i < len(old_shape):
        ...
        if new_shape[i] == 1 and old_shape[old_shape_i] != 1:
            ...
            if (
                ...
                or prod([old_shape[x] for x in axis_groups[i]]) * old_shape[old_shape_i] > new_shape[i]
            ):
                return None
