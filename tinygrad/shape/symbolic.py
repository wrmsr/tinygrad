...
class Node:
    ...
    def __ge__(self, b: int):
        return create_opnode(GeNode, self, b)

    def __lt__(self, b: int):
        return create_opnode(LtNode, self, b)
    ...
    def __mul__(self, b: int):
        ...
        if isinstance(self, MulNode):
            return self.a * (self.b * b)  # two muls is one mul
    ...
    def sum(nodes: List[Node]) -> Node:
        ...
        mul_nodes = sorted(
            mul_nodes, key=lambda x: x.a.render()
        )  # group by equality (ugh, uses render!)
        new_nodes = [
            k * sum(x.b for x in g)
            for k, g in itertools.groupby(mul_nodes, key=lambda x: x.a)
        ]
        nodes = [
            x if not isinstance(x, MulNode) or x.b != 1 else x.a for x in new_nodes
        ]

        # filter 0s
        nodes = [x for x in nodes if x.min != 0 or x.max != 0]
        return (
            create_rednode(SumNode, nodes)
            if len(nodes) > 1
            else (nodes[0] if len(nodes) == 1 else NumNode(0))
        )
...
def create_opnode(typ: Type[OpNode], a: Node, b: int):
    ...
    if typ == GeNode:
        ...
        ret.min, ret.max = int(a.max < b), int(a.min < b)
    elif typ == MulNode:
        ret.min, ret.max = (a.min * b, a.max * b) if b >= 0 else (a.max * b, a.min * b)
    elif typ == DivNode:
        assert a.min >= 0
