...
class Node:
    ...
    def __ge__(self, b: int):
        return create_node(GeNode(self, b))
    ...
    def sum(nodes: List[Node]) -> Node:
        ...
        mul_groups: Dict[str, Tuple[Node, List[MulNode]]] = defaultdict(
            lambda: (Node(), [])
        )
        ...
        new_nodes = [
            x if not isinstance(x, MulNode) or x.b != 1 else x.a for x in mul_nodes
        ]
        ...
        return (
            ...
            if len(new_nodes) > 1
            else (new_nodes[0] if len(new_nodes) == 1 else NumNode(0))
        )
...
class GeNode(OpNode):
    ...
    def __floordiv__(self, b: int, _=False):
        return (self.a // b) >= (self.b // b)

    def get_bounds(self) -> Tuple[int, int]:
        return int(self.a.min >= self.b), int(self.a.max >= self.b)
...
class LtNode(OpNode):
    ...
    def get_bounds(self) -> Tuple[int, int]:
        return int(self.a.max < self.b), int(self.a.min < self.b)
...
class MulNode(OpNode):
    ...
    def __floordiv__(
        self, b: int, factoring_allowed=False
...
class SumNode(RedNode):
    def __mul__(self, b: int):
        return Variable.sum([x * b for x in self.nodes])  # distribute mul into sum

    def __floordiv__(self, b: int, factoring_allowed=True):
        if not factoring_allowed:
            return Node.__floordiv__(self, b, factoring_allowed)
        factors, tmp_nofactor = partition(
            self.nodes, lambda x: (isinstance(x, (MulNode, NumNode))) and x.b % b == 0
        )
        nofactor = []
        ...
        for x in tmp_nofactor:
            if isinstance(x, NumNode):
                if (x.b % b) != x.b:
                    factors.append(
                        Variable.num(x.b - (x.b % b))
                    )  # python does floor division
                nofactor.append(Variable.num(x.b % b))
            else:
                nofactor.append(x)
        gcd = [
            math.gcd(x.b, b) if isinstance(x, (MulNode, NumNode)) else None
            for x in nofactor
        ]
        if len(factors) > 0:
            ...
            if len(gcd) > 0 and all_same(gcd) and gcd[0] is not None and gcd[0] > 1:
                nofactor_term = Variable.sum(
                    [
                        (x.a * (x.b // gcd[0]))
                        if isinstance(x, MulNode)
                        else Variable.num(x.b // gcd[0])
                        for x in nofactor
                    ]
                ) // (b // gcd[0])
            else:
...
class AndNode(RedNode):
    def __mul__(self, b: int):
        Variable.ands([x * b for x in self.nodes])

    def __floordiv__(self, b: int, _=True):
