...
MERGE_ONE_REDUCE_INTO_ELEMENTWISE, SHUFFLE_PAD_OPS = (
    OPT >= 2,
    OPT >= 2,
)  # shuffle pad ops is fine now since we only push to merge binops
...
def _ast_reduceops(self: LazyBuffer) -> LazyOp:
    ...
    if (
        MERGE_ELEMENTWISE_INTO_REDUCE
        and src.realized is None
        and src.optype == BinaryOps
...
def _ast_binaryops(self: LazyBuffer) -> LazyOp:
    real_srcs: Dict[LazyBuffer, Union[None, LazyOp, LazyBuffer]] = {
        x: None for x in get_buffers(self.op)
    }
    ...
    if len(psrcs) >= 1 and MERGE_ONE_REDUCE_INTO_ELEMENTWISE:
        ...
        if psrc[0].shape != psrc[1].shape:
            ...
            assert (
                psrc[0].shape == self.shape
            ), f"shape mismatch {psrc[0].shape} != {self.shape}"
    ...
    return (
        LazyOp(MovementOps.RESHAPE, (ast,), self.shape)
...
def get_weakop(op: LazyOp) -> LazyOp:
    return LazyOp(
        op.op,
        tuple(
            get_weakop(x) if isinstance(x, LazyOp) else weakref.ref(x) for x in op.src
        ),
        op.arg,
    )
...
def get_movementroot(root: LazyBuffer, allow_contiguous=False) -> LazyBuffer:
    return (
        get_movementroot(root.op.src[0], allow_contiguous)
        if root.realized is None
        and (
...
def get_movementroot_contiguous(x: LazyBuffer) -> LazyBuffer:
    return (
        get_movementroot_contiguous(x.op.src[0])
...
def replace_with_movement_ops(
    y: Union[LazyOp, LazyBuffer], ops: List[Tuple[MovementOps, Tuple[Any, ...]]]
) -> LazyBuffer:
...
lazycache: WeakValueDictionary[
    Tuple[str, DType, OpType, LazyOp], LazyBuffer
] = WeakValueDictionary()
...
def create_lazybuffer(
    ...
    if optype == LoadOps and op.op in [LoadOps.FROMCPU, LoadOps.EMPTY]:
...
class LazyBuffer:
    ...
    def __init__(
        self, device: str, st: ShapeTracker, optype: OpType, op: LazyOp, dtype: DType
    ):
        self.st = (
            st  # NOTE: this is not a copy! this should be a "read-only" ShapeTracker
        )
        self.device, self.shape, self.optype, self.dtype = (
            device,
            self.st.shape,
            optype,
            dtype,
    ...
    def _device_extra_args(self) -> Dict[str, str]:
        return {"device": self.device.split(":")[1]} if ":" in self.device else {}

    def realize(self: LazyBuffer) -> LazyBuffer:
        if self.realized is None:
            # get real ops first
            if self.op.op == LoadOps.FROMCPU:
                if prod(self.op.arg.shape) == 1 and hasattr(
                    Device[self.device].codegen, "supports_constant_folding"
                ):
                    self.realized = RawConst(
                        1, dtypes.from_np(self.op.arg.dtype), self.op.arg().flatten()[0]
                    )
                else:
                    if DEBUG >= 4:
                        print(
                            f"copying {self.op.arg.shape}:{dtypes.from_np(self.op.arg.dtype)} -> {self.device}"
                        )
                    self.realized = Device[self.device].buffer.fromCPU(
                        self.op.arg(), **self._device_extra_args()
                    )
            elif self.op.op == LoadOps.EMPTY:
                self.realized = Device[self.device].buffer(
                    prod(self.shape), self.dtype, **self._device_extra_args()
                )
            elif self.op.op == LoadOps.CONTIGUOUS:
                realized = self.op.src[0].realize().realized
                if (
                    self.op.src[0].st.contiguous
                    and not isinstance(realized, RawConst)
                    and realized.size == prod(self.shape)
                ):
            ...
            if self.realized is None:
                ...
                if (
                    self.optype != MovementOps
                    and isinstance(self.dtype, ImageDType)
                    and (
                ...
                self.realized = Device[self.device].exec_ast(
                    self.op, output=self, **self._device_extra_args()
                )

            assert isinstance(
                self.realized, (RawConst, Device[self.device].buffer)
    ...
    def empty(shape, dtype, device) -> LazyBuffer:
        return create_lazybuffer(
            device, shape, LoadOps, LazyOp(LoadOps.EMPTY, tuple()), dtype
        )
    ...
    def const_like(self, val) -> LazyBuffer:
        return (
            create_lazybuffer(
                ...
                LoadOps,
                LazyOp(
                    LoadOps.FROMCPU, tuple(), LazyNumpyArray([val], (1,), self.dtype.np)
                ),
                self.dtype,
            )
    ...
    def toCPU(self):
        realized = (
            self.cast(dtypes.from_np(self.dtype.np)).contiguous().realize().realized
        )
    ...
    def cast(self: LazyBuffer, arg: DType) -> LazyBuffer:
        return (
            elementwise_op(UnaryOps.CAST, self, arg=arg) if self.dtype != arg else self
        )
    ...
    def contiguous(self: LazyBuffer) -> LazyBuffer:
        ...
        return create_lazybuffer(
            ...
            self.shape,
            LoadOps,
            LazyOp(LoadOps.CONTIGUOUS, (self,)),
            self.dtype,
        )

    def reduce_op(
        self: LazyBuffer, op: ReduceOps, new_shape: Tuple[int, ...]
    ) -> LazyBuffer:
    ...
    def movement_op(
        self: LazyBuffer, op: MovementOps, arg: Tuple[Any, ...]
    ) -> LazyBuffer:
        # very instant nop
        ...
        if self.realized is None and self.op.op == op:
            ...
            if op == MovementOps.PERMUTE:
                return self.op.src[0].movement_op(
                    op, tuple(self.op.arg[i] for i in arg)
                )
            ...
            if op == MovementOps.STRIDE:
                return self.op.src[0].movement_op(
                    op, tuple(i * j for i, j in zip(arg, self.op.arg))
                )
        ...
        if op == MovementOps.PERMUTE and local_st.contiguous:
            return self.movement_op(
                MovementOps.RESHAPE, tuple(self.shape[i] for i in arg)
            )
        ...
        if (
            op == MovementOps.PERMUTE
            and self.realized is None
            and self.op.op == MovementOps.EXPAND
        ):
            self.op.src[0].children.discard(self)
            return (
                self.op.src[0]
        ...
        if (
            ...
            if shape_idx_groups := get_contraction(self.op.src[0].shape, self.shape):
                ...
                return (
                    ...
                    .movement_op(
                        MovementOps.PERMUTE,
                        tuple(flatten(shape_idx_groups[i] for i in arg)),
                    )
                    .movement_op(
                        MovementOps.RESHAPE,
                        ShapeTracker(self.st).movement_op(op, arg).shape,
                    )
                )
        ...
        ):  # and op != MovementOps.EXPAND and (op != MovementOps.PAD or (SHUFFLE_PAD_OPS and all(x.op != BinaryOps.DIV for x in get_lazyops(self.op)))):
        ...
        if (
            REMOVE_MOVEMENT_NOPS
            and ret.realized is None
            and self.realized is None
            and ret.st.contiguous
        ):
            ...
            if (
                root.st.contiguous
                and root != self
                and prod(ret.st.shape) == prod(root.shape)
            ):
...
def elementwise_op(
    op: Union[UnaryOps, BinaryOps], *srcs: LazyBuffer, arg: Optional[Any] = None
) -> LazyBuffer:
    ...
    out_device, out_shape, out_dtype = (
        srcs[0].device,
        srcs[0].shape,
        max(x.dtype for x in srcs) if op != UnaryOps.CAST else cast(DType, arg),
    )
