...
from tinygrad.ops import Interpreted
...
from tinygrad.shape.shapetracker import get_contraction
...
SHUFFLE_PAD_OPS = OPT >= 2  # shuffle pad ops is fine now since we only push to merge binops
PUSH_PERMUTES = OPT >= 3
...
def _ast_binaryops(self: 'LazyBuffer') -> LazyOp:
    real_srcs: ta.Dict[LazyBuffer, ta.Union[None, LazyOp, LazyBuffer]] = {x: None for x in get_buffers(self.op)}

    # NOTE: contiguous does not always mean the same size with SHRINK. this is still mergeable but requires more thought how
    # TODO: this can also support late fusion of BinaryOps, required for test_fold_conv_sgd
    psrcs: ta.List[ta.Tuple[LazyBuffer, LazyBuffer]] = [
        (k, x)
    ...
    intermediate_shape: ta.Tuple[int, ...] = self.shape
...
def get_movementroot(root: 'LazyBuffer', allow_contiguous=False) -> 'LazyBuffer':
    if root.realized is None and (
        root.optype == MovementOps or (
        root.op.op == LoadOps.CONTIGUOUS and allow_contiguous and root.op.src[0].st.contiguous)
    ):
        return get_movementroot(root.op.src[0], allow_contiguous)
    return root


def get_movementroot_contiguous(x: 'LazyBuffer') -> 'LazyBuffer':
    if x.realized is None and x.op.op == LoadOps.CONTIGUOUS:
        return get_movementroot_contiguous(x.op.src[0])
...
def replace_with_movement_ops(
    y: ta.Union[LazyOp, 'LazyBuffer'],
    ops: ta.List[ta.Tuple[MovementOps, ta.Tuple[ta.Any, ...]]],
) -> 'LazyBuffer':
...
lazycache: ta.MutableMapping[ta.Tuple[str, DType, OpType, LazyOp], 'LazyBuffer'] = weakref.WeakValueDictionary()
...
class LazyBuffer:
    ...
    def __init__(
        ...
        self.realized: ta.Optional[RawBuffer] = None
    ...
    def _device_extra_args(self) -> ta.Dict[str, int]:
    ...
    def realize(self: 'LazyBuffer') -> 'LazyBuffer':
        if self.realized is None:
            ...
            if self.realized is None:
                ...
                if self.optype != MovementOps and isinstance(self.dtype, ImageDType) and (
                    ...
                    if self.op.op == MovementOps.RESHAPE:
                        ...
                        self.op = LazyOp(
                            MovementOps.RESHAPE,
                            (LazyOp(UnaryOps.CAST, self.op.src, dtypes.float32),),
                            self.op.arg,
                        )
                    else:
            ...
            assert isinstance(self.realized, (RawConst, Device[self.device].buffer)), \
                f"device mismatch on realized got {type(self.realized)} expected {self.device}"

            # HACK: allow hot casting of images
            assert self.realized.dtype == self.dtype or self.dtype.name.startswith("image"), \
                f"dtype mismatch on realize got {self.realized.dtype} expected {self.dtype}"
    ...
    # NOTE: we have to make a copy of the numpy array here in case the user changes it. expose this? LazyNumpyArray
    ...
    def fromCPU(x: LazyNumpyArray, device) -> 'LazyBuffer':
        return create_lazybuffer(
            ...
            x.shape,
            LoadOps,
            LazyOp(LoadOps.FROMCPU, tuple(), x),
    ...
    def const_like(self, val) -> 'LazyBuffer':
        return create_lazybuffer(
            ...
            .movement_op(MovementOps.RESHAPE, (1,) * len(self.shape), ) \
            .movement_op(MovementOps.EXPAND, self.shape, )

    # NOTE: we also have to copy the numpy array on the way out...otherwise the underlying Tensor could be freed and use
    # after free. improve this?
    def toCPU(self):
        realized = self.cast(dtypes.from_np(self.dtype.np)).contiguous().realize().realized
        ret = ta.cast(RawBuffer, realized).toCPU().reshape(self.shape)
        return ret
    ...
    def reduce_op(self: 'LazyBuffer', op: ReduceOps, new_shape: ta.Tuple[int, ...]) -> 'LazyBuffer':
    ...
    def movement_op(self: 'LazyBuffer', op: MovementOps, arg: ta.Tuple[ta.Any, ...]) -> 'LazyBuffer':
        # very instant nop
        ...
        if (
            ...
            if shape_idx_groups := get_contraction(self.op.src[0].shape, self.shape):
                ...
                return self.op.src[0] \
                    .movement_op(MovementOps.PERMUTE, tuple(flatten(shape_idx_groups[i] for i in arg))) \
                    .movement_op(MovementOps.RESHAPE, ShapeTracker(self.st).movement_op(op, arg).shape)

        # if this MovementOp is being applied to a BinaryOp, apply the MovementOp to all the BinaryOp inputs instead.
        # NOTE: UnaryOps is never an OpType
        if (
            ...
            and (
                ...
                op == MovementOps.RESHAPE and self.op.op in UnaryOps)
            )
            and len(self.children) == 0
        ):
            # and op != MovementOps.EXPAND
            # and (op != MovementOps.PAD or (SHUFFLE_PAD_OPS and all(x.op != BinaryOps.DIV for x in get_lazyops(self.op))))  # noqa
...
def _push_movement_ops(srcs: ta.Tuple[LazyBuffer, ...]) -> ta.Tuple[LazyBuffer, ...]:
    new_srcs = []

    for x in srcs:
        mops: ta.List[ta.Tuple[MovementOps, ta.Tuple[ta.Any, ...]]] = []
        bx = x
        ...
        while (
            ...
            and (bx.op.op != MovementOps.PAD or SHUFFLE_PAD_OPS)
        ...
        if (
            bx.realized is None
            and bx.optype == BinaryOps
            and len(bx.children) <= 1
            and len(mops)
            and (
                all(x[0] != MovementOps.PAD for x in mops)
                or all(x.op != BinaryOps.DIV for x in get_lazyops(bx.op))
            )
        ):
...
def elementwise_op(op: ta.Union[UnaryOps, BinaryOps], *srcs: LazyBuffer, arg: ta.Optional[ta.Any] = None) -> LazyBuffer:
    ...
    out_dtype = max(x.dtype for x in srcs) if op != UnaryOps.CAST else ta.cast(DType, arg)
    ...
    if MERGE_ELEMENTWISE_OPS:
        ...
        srcs = tuple(x.op if x.optype == BinaryOps and len(x.children) == 0 and x.realized is None else x for x in srcs)
...
class _Device:
    ...
    def _get_device(self, x: str) -> ta.Union[Interpreted, Compiled]:
        return [
            cls
