...
render_cl[
    ...
    lambda self, ops, ctx: f"({'&&'.join(sorted([x.render(ops, ctx) for x in self.nodes]))})"
)

NATIVE_EXPLOG = getenv(
    "NATIVE_EXPLOG", 0
)  # this is needed as a switch for the tests to pass
...
def uops_to_cstyle(
    ...
    for uop, newvar, vin, args in uops:
        if uop == UOps.LOOP:
                ...
                if isinstance(var, NumNode):
                    ...
                    if args[1] == "global" and lang.gid:
                        if len(args[0]) >= 4 and len(args[0]) - i > 2:
                            ...
                            if i == 0:
                                kk(
                                    f"{{ int {var.expr} = {lang.gid[-1]};  /* {var.max + 1} */"
                                )
                                root = var.expr
                                global_size.append(var.max + 1)
                            else:
                                kk(
                                    f"{{ int {var.expr} = {root} % {var.max + 1}; {root} /= {var.max + 1};"
                                )
            ...
            uop == UOps.LOAD and newvar is not None and newvar.ltype == LocalTypes.float
        ):
            assert not isinstance(
                bufs[args.i].dtype, ImageDType
            ), "image load must be float4"
            ...
            if bufs[args.i] is not None and isinstance(bufs[args.i].realized, RawConst):
                ...
                if lang.uses_vload and bufs[args.i].dtype == dtypes.float16:
                    val = (
                        f"vload_half({args.idx.render(render_cl)}, {bufnames[args.i]})"
                    )
                else:
            ...
            if args.valid.min == 1:
                ...
                kk(
                    f"float {newvar.name} = ({args.valid.render(render_cl)}) ? ({val}) : 0.0f;"
                )
        elif (
            uop == UOps.LOAD
            and newvar is not None
            and newvar.ltype == LocalTypes.float4
        ):
            ...
            vin[0].ltype == LocalTypes.float
            or (vin[0].ltype == LocalTypes.float4 and vin[0].offset is not None)
        ):
            assert not isinstance(
                bufs[args.i].dtype, ImageDType
            ...
            uop == UOps.STORE
            and len(vin) != 0
            and vin[0].ltype == LocalTypes.float4
...
class CStyleCodegen(Linearizer):
    ...
    def codegen(self):
        ...
        for i, s in enumerate(local_size):
            global_size[i] *= s

        # painfully name the function something unique
        if prg in CStyleCodegen.kernel_name_cache:
            function_name, display_name = CStyleCodegen.kernel_name_cache[prg]
        else:
            CStyleCodegen.kernel_cnt[self.function_name] += 1
            suffix = (
                f"{'n' + str(CStyleCodegen.kernel_cnt[self.function_name] - 1)}"
                if CStyleCodegen.kernel_cnt[self.function_name] > 1
                else ""
            )
            CStyleCodegen.kernel_name_cache[prg] = (
                function_name,
                display_name,
            ) = self.function_name + suffix, self.display_name + colored(
                suffix, "black", bright=True
            )

        return ASTRunner(
            function_name,
            prg.replace("KERNEL_NAME_PLACEHOLDER", function_name),
            global_size[::-1] if len(global_size) else [1],
            local_size[::-1] if len(local_size) else None,
