...
def uops_to_cstyle(
    ...
    buftypes = [
        (
            ...
            else ("const " if i > 0 else "") + lang.buffer_prefix + x.dtype.name + "*" + lang.buffer_suffix
        )
        for i, x in enumerate(bufs)
        if not isinstance(x, LocalBuffer)
...
class CStyleCodegen(Linearizer):
    ...
    def codegen(self):
        ...
        if prg in CStyleCodegen.kernel_name_cache:
            ...
            suffix = f"{'n' + str(CStyleCodegen.kernel_cnt[self.function_name] - 1)}" \
                if CStyleCodegen.kernel_cnt[self.function_name] > 1 else ""
            CStyleCodegen.kernel_name_cache[prg] = function_name, display_name = \
                self.function_name + suffix, self.display_name + colored(suffix, 'black', bright=True)

        return ASTRunner(
            function_name, prg.replace("KERNEL_NAME_PLACEHOLDER", function_name),
