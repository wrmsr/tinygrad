...
from tinygrad.helpers import mnum
from tinygrad.helpers import prod
from tinygrad.lazy import LazyBuffer
from tinygrad.ops import BinaryOps
from tinygrad.ops import FlopCounter
...
from tinygrad.ops import map_buffers
...
class LocalTypes(enum.Enum):
    float = enum.auto()
    float4 = enum.auto()
    half = enum.auto()
    half4 = enum.auto()
    simdgroup_float8x8 = enum.auto()
...
class Token(ta.NamedTuple):
    ...
    def __repr__(self):
        return (
            f"<{self.name}>" if self.offset is None and self.ltype == LocalTypes.float else
            f"<{self.name}:{self.ltype.name}:{self.offset}>"
        )


# TODO: the next three functions are poorly written
def get_grouped_float4_idxs(acc: ta.List[Token]) -> ta.Optional[ta.List[int]]:
...
def to_float4(x: ta.List[Token]) -> ta.Optional[Token]:
    if all_same(x):
        return x[0]
...
def get_grouped_maybe_float4(*values: ta.List[Token], grouping_allowed=True):
...
class UOp(ta.NamedTuple):
    ...
    def __repr__(self):
        return (
            f"{str(self.uop):20s}: "
            f"{str(self.out) if self.out is not None else '':25s} "
            f"{str(self.vin):32s} "
            f"{self.arg}"
        )
...
class Linearizer:
    ...
    def __init__(self, ast: LazyOp, output_buffer: LazyBuffer):
        ...
        # key for lookup in cache (can change, str might not be right). bufs are needed because kernels like
        # f(x) = x + x and f(x, y) = x + y have the same str(ast), but are different kernels. mapping the buffers to
        # integers is required because a-b != b-a (and how would you tell a and b apart?)
        self.key = f"ASTKernelKey ast={str(map_buffers({x: i for i, x in enumerate(self.bufs)}, ast))} bufs={self.bufs}"

    info: FlopCounter
    mem_estimate: int
    full_buf_index: int
    group_for_reduce: ta.List[int]
    upcasted: int
    ...
    def acc_offsets(self, i):
        if self.upcasted == 0:
            return [0]
        acc_strides = [
            x * (1 - self.upcasted_axis(i)[::-1][i][2])
            for i, x in enumerate(strides_for_shape(
                tuple(1 if r else s for s, _, r in self.upcasted_axis(i)[::-1])
            ))
        ]
        return [
            sum(t)
            for t in itertools.product(*[
                ...
                for i, x in enumerate(self.upcasted_axis(i)[::-1])
            ])
        ]
    ...
    def global_load(self, i, idxs: ta.List[Variable], const=None) -> ta.List[Token]:
        load_offset: ta.Dict[ta.Tuple[int, ...], ta.Any] = {
            uidxs: (LocalTypes.float, uidxs) + self.sts[i].expr_idxs(idxs + [Variable.num(x) for x in uidxs[::-1]])
        ...
        if should_upcast:
            ...
            for k, out_tokens in self._group_float4(i, load_offset).items():
                ...
                valids_okay = (
                    all_same([x[3] for x in out_tokens]) or
        ...
        for uidxs, (localtype, uidx_list, idx, valid) in load_offset.items():
            ...
            if key not in cache:
                cache[key] = self.uop(
                    UOps.LOAD,
                    Token(f"val{mnum(i)}_{len(cache)}", localtype),
                    [],
                    MemOp(i, idx, valid)
                ) if const is None else self.uop(
                    UOps.CONST,
                    Token(f"acc{mnum(i)}_{len(cache)}",localtype),
                    [],
                    const,
                )
    ...
    def global_store(self, i, idxs: ta.List[Variable], store: ta.List[Token], ssa) -> None:
        store_offset: ta.Dict[ta.Tuple[int, ...], Token] = dict(zip(self.shape_offsets(i), store))
    ...
    def linearize(self):
        ...
        if self.group_for_reduce:
            # NOTE: this is assuming the global size = the local size in these dims. in general, this doesn't have to be
            # true
            local_idxs = [
                Variable(f"lidx{i}", 0, self.full_shape[i] - 1 if i >= self.first_reduce else 0)
        ...
        if self.reduceop is not None:
            ...
            reduce_idxs = [
                ...
                for i in range(self.first_reduce + len(self.group_for_reduce), self.shape_len - self.upcasted)
            ]
            ...
            acc = self.global_load(
                ...
                {ReduceOps.SUM: 0.0, ReduceOps.MAX: -math.inf}[ta.cast(ReduceOps, self.reduceop.op)],
            )
            ...
            loaded_buffers.update({
                ...
                if b in self.earlybufs and i != 0
            })
            ...
            if self.group_for_reduce:
                ...
                acc = self.global_load(
                    -1,
                    local_idxs[:removed] + fake_reduce_idxs,
                    {ReduceOps.SUM: 0.0, ReduceOps.MAX: -math.inf}[ta.cast(ReduceOps, self.reduceop.op)],
                )

                # late reduce loop
                end_local_idxs = [
                    Variable(f"tidx{i}", 0, self.full_shape[i] - 1 if i >= self.first_reduce else 0)
                    for i in range(0, self.first_reduce + len(self.group_for_reduce))
                ]
        ...
        # load latebufs
        loaded_buffers.update({
            b: self.global_load(i, global_idxs[:removed] + fake_reduce_idxs)
            for i, b in enumerate(self.bufs)
            if b not in self.earlybufs
            and i != 0
            and not isinstance(b, LocalBuffer)
        })
    ...
    def uop(self, uop: UOps, out: _OT, vin: ta.List[Token], arg: ta.Any = None) -> _OT:
        self.uops.append(UOp(uop, ta.cast(ta.Optional[Token], out), vin, arg))
        if DEBUG >= 4:
            print(self.uops[-1])
        return out

    def ast_parse(self, x, acc, loaded_buffers, ssa, do_reduce=False) -> ta.List[Token]:
        ...
        if isinstance(x.op, (ReduceOps, FusedOps)):
            ret = [
                ...
                for idx, val in get_grouped_maybe_float4(acc, *values)
            ]
            ...
            ret = [
                (
                    ...
                    self.uop(
                        UOps.ALU,
                        ssa('alu', LocalTypes.float4)
                        if any(x.ltype == LocalTypes.float4 and x.offset is None for x in val) else
                        ssa('alu'),
                        list(val),
                        x.op,
                    ),
                )
                for idx, val in get_grouped_maybe_float4(*values, grouping_allowed=x.op != BinaryOps.CMPEQ)
            ]

        ordered_ret: ta.List[ta.Optional[Token]] = [None] * len(values[0])
        # scatter
        for i, j in ret:
        ...
        return ta.cast(ta.List[Token], ordered_ret)
    ...
    @property
    def full_shape(self) -> ta.Tuple[int, ...]:
        return self.sts[self.full_buf_index].shape

    @property
    def full_unupcasted_shape(self) -> ta.Tuple[int, ...]:
    ...
    def colors(self) -> ta.List[str]:
        ...
        # (green)
        colors += [
            "green" if i in self.upcast_in_mid_reduce_axes else "cyan"
            for i in range(self.first_reduce, self.first_reduce + len(self.group_for_reduce))
        ]
    ...
    def printbufs(self, prefix=""):
        for i in range(len(self.sts)):
            print(
                prefix,
                f"{i:3d} "
                f"{str(self.bufs[i].realized) if self.bufs[i] is not None else 'FAKE':47s}",
                self.sts[i].views
            )
    ...
    def hand_coded_optimizations(self):
        ...
        if self.bufs[0].dtype.name.startswith('image') and not self.float4_axis(
            ...
            if self.sts[0].shape[axes[0]] % 4 == 0:
                ...
                self.shift_to(axes[0], 4, insert_before=self.first_reduce + len(self.group_for_reduce))
        ...
        if self.bufs[0].dtype.name.startswith('image') and self.first_reduce + len(self.group_for_reduce) < 3:
            ...
            if (
                (base_shape[0] * base_shape[1]) % self.sts[0].shape[0] == 0 and
                self.sts[0].shape[0] // base_shape[0] != 0
            ):
                if DEBUG >= 4:
        ...
        while prod(self.sts[0].shape[:self.first_reduce]) >= 1024:
            ...
            for axis, upcast_amount in itertools.product(range(self.first_reduce), [3, 4]):
                ...
                if (
                    ...
                    any(
                        self.sts[buf_index].views[-1].strides[axis] == 0 and
        ...
        if (
            self.first_reduce < (self.shape_len - self.upcasted) and
            (
                len(list(self.shape_offsets(self.full_buf_index))) <= 4 or
                not any(r for _, _, r in self.upcasted_axis(self.full_buf_index))
            )
        ):
