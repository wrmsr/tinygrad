...
class Token(NamedTuple):
    ...
    def __repr__(self):
        return (
            f"<{self.name}>"
            if self.offset is None and self.ltype == LocalTypes.float
            else f"<{self.name}:{self.ltype.name}:{self.offset}>"
        )
    ...
    for i, a in enumerate(acc):
        ...
        if a.ltype == LocalTypes.float4 and a.offset == 0:
            ...
            for j, b in enumerate(acc):
                ...
                if (
                    a.name == b.name
                    and b.ltype == LocalTypes.float4
                    and b.offset == len(friends) + 1
                ):
                    friends.append(j)
            if len(friends) == 3:
                idxs += friends
...
def get_grouped_maybe_float4(*values: List[Token], grouping_allowed=True):
    ...
    ), f"all values are not the same length {values}"
...
class Linearizer:
    ...
    def process(self) -> None:
        ...
        self.mem_estimate: int = sum(
            x.dtype.itemsize
            * (x.realized.size if x.realized is not None else prod(x.shape))
            for x in self.bufs
            if x is not None
        )
        ...
        self.full_buf_index: int = (
            self.bufs.index(self.earlybufs[0]) if len(self.earlybufs) > 0 else 0
        )
    ...
    def float4_axis(self, i):
        return [
            ...
            if x >= self.shape_len - self.upcasted and self.sts[i].shape[x] == 4
        ]
    ...
    def upcasted_axis(self, i):
        return list(
            zip(
                ...
                self.sts[i]
                .views[-1]
                .strides[self.shape_len - self.upcasted :],  # WRONG
                [
                    x != y
                    for x, y in zip(
    ...
    def global_load(self, i, idxs: List[Variable], const=None) -> List[Token]:
        load_offset: Dict[Tuple[int, ...], Any] = {
            uidxs: (LocalTypes.float, uidxs)
            + self.sts[i].expr_idxs(idxs + [Variable.num(x) for x in uidxs[::-1]])
            for uidxs in self.shape_offsets(i)
        }

        # float4 grouping (optional)
        should_upcast = (
            self.supports_float4
            and self.bufs[i].dtype != dtypes.float16
        ...
        if should_upcast:
            ...
            for k, out_tokens in self._group_float4(i, load_offset).items():
                ...
                valids_okay = all_same([x[3] for x in out_tokens]) or (
                    all_same([x[3] // 4 for x in out_tokens])
                    and (out_tokens[0][3] // 4) * 4 == out_tokens[0][3]
                )
                if (
                    any(
                        idx.min != idx.max or idx.min != val
                        for idx, val in zip(idxs, range(4))
                    )
                    or (out_tokens[0][2] // 4) * 4 != out_tokens[0][2]
                    or not valids_okay
                ):
    ...
    def global_store(self, i, idxs: List[Variable], store: List[Token], ssa) -> None:
        store_offset: Dict[Tuple[int, ...], Token] = dict(
            zip(self.shape_offsets(i), store)
        )
        ...
        if should_upcast:
            ...
            for k, out_tokens in self._group_float4(i, store_offset).items():
                if all_same([x.name for x in out_tokens]) and tuple(range(4)) == tuple(
                    x.offset for x in out_tokens
                ):
                    store_offset_new[k] = Token(out_tokens[0].name, LocalTypes.float4)
                else:
                    store_offset_new[k] = self.uop(
                        UOps.CAST, ssa("alu", LocalTypes.float4), out_tokens
                    )
        ...
        for uidxs, var in store_offset.items():
            self.uop(
                ...
                MemOp(
                    i,
                    *self.sts[i].expr_idxs(
                        idxs + [Variable.num(x) for x in uidxs[::-1]]
                    ),
                ),
            )
    ...
    def linearize(self):
        ...
        self.function_name = ("r_" if self.reduceop else "E_") + "_".join(
            [str(x) for x in self.full_shape]
        )
        ...
        global_idxs = [
            Variable(
                f"gidx{i}", 0, self.full_shape[i] - 1 if i < self.first_reduce else 0
            )
        ...
        if self.reduceop is not None:
            ...
            if self.group_for_reduce:
                self.global_store(
                    -1, local_idxs + fake_reduce_idxs, acc, ssa
                )  # store accumulators
                self.uop(
                    UOps.ENDLOOP, None, [], (local_idxs, "local")
                )  # this is a barrier on GPUs
                ...
                loaded_buffers["LOCAL_BUFFER"] = self.global_load(
                    -1, end_local_idxs + fake_reduce_idxs
                )
        ...
        loaded_buffers.update(
            {
                ...
                for i, b in enumerate(self.bufs)
                if b not in self.earlybufs and i != 0 and not isinstance(b, LocalBuffer)
            }
        )
    ...
    def ast_parse(self, x, acc, loaded_buffers, ssa, do_reduce=False) -> List[Token]:
        ...
        if x.op in [UnaryOps.NOOP, UnaryOps.CAST]:
            return self.ast_parse(
                x.src[0], acc, loaded_buffers, ssa
            )  # cast isn't an ALU op
        if x.op in ReduceOps and not do_reduce:
            return acc
        # MULACC fusion. TODO: this is copied from Interpreted
        if (
            x.op == ReduceOps.SUM
            and isinstance(x.src[0], LazyOp)
        ...
        if (
            ...
            and isinstance(x.src[0].src[0], LazyOp)
            ...
            ret = [
                (
                    ...
                    self.uop(
                        ...
                        if any(
                            x.ltype == LocalTypes.float4 and x.offset is None
                            for x in val
                        )
                        else ssa("alu"),
                        list(val),
                ...
                for idx, val in get_grouped_maybe_float4(
                    *values, grouping_allowed=x.op != BinaryOps.CMPEQ
                )
            ]
        ...
        for i, j in ret:
            for o, k in enumerate(i):
                ordered_ret[k] = (
                    Token(j.name, j.ltype, o) if j.ltype == LocalTypes.float4 else j
                )
        assert all(
            isinstance(x, Token) for x in ordered_ret
    ...
    def printbufs(self, prefix=""):
        for i in range(len(self.sts)):
            print(
                prefix,
                f"{i:3d} {str(self.bufs[i].realized) if self.bufs[i] is not None else 'FAKE':47s}",
                self.sts[i].views,
            )
        print(
            " ".join(
                colored(f"{s:4d}", color)
                for s, color in zip(self.full_shape, self.colors())
            )
        )
    ...
    def simplify_ones(self):
        ...
        all_ones = [
            all(st.shape[i] == 1 for st in self.sts) for i in range(self.shape_len)
        ]
        # keep at least 1 one
        if all(all_ones):
            all_ones[-1] = False
    ...
    def simplify_merge_adjacent(self):
        ...
        shapes, strides = [x.shape for x in self.sts], [
            x.views[-1].strides for x in self.sts
        ]
    ...
    def required_optimizations(self, early_only=False):
        for buf_index, buf in enumerate(self.bufs):
            ...
            if (not early_only or buf in self.earlybufs) and isinstance(
                self.bufs[buf_index].dtype, ImageDType
            ):
