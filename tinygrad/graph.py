...
def log_op(
    ret: LazyBuffer, ast: LazyOp, show_graph: Optional[bool] = None, phantom=False
):
    ...
    inp: List[LazyBuffer] = [
        x for x in get_buffers(ast) if not isinstance(x.realized, RawConst) or GRAPH > 1
    ]
    ...
    if DEBUG >= 6:
        print(
            f"{op} : {', '.join([f'{x.shape}-<{nm(x)}>' for x in inp])} -> {ret.shape}-<{nm(ret)}>"
        )
    if show_graph:
        top_colors = {
            LoadOps: "#FFFF80",
        ...
        dashed = (optype == LoadOps and hasattr(ret, "_backing")) or (
            hasattr(ret, "st") and not ret.st.contiguous
        )  # type: ignore
        ...
        for x in inp:
            G.add_edge(
                ...
                nm(ret),
                label=get_sop(op),
                color="#00000060" if phantom else "black",
            )
    ...
    for n in G.nodes:
        if "prunable" in G.nodes[n] and G.nodes[n]["prunable"]:
            G.add_edges_from(
                [
                    (x, y)
                    for (x, _), (_, y) in itertools.product(
