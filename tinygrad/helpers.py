...
class Context:
    ...
    def __enter__(self):
        ContextVar.ctx_stack.append(
            {
                **self.pvars,
                **{
                    key: ContextVar.ctx_stack[-1][key]
                    for key in ContextVar.ctx_stack[-1].keys()
                    if key not in self.pvars
                },
            }
        )

    def __exit__(self, *args):
        ContextVar.ctx_stack.pop()
...
class ContextVar:
    ...
    def __init__(self, key, default_value):
        ...
        if key not in ContextVar.ctx_stack[-1]:
            ContextVar.ctx_stack[-1][key] = self.initial_value

    def __call__(self, x):
        ContextVar.ctx_stack[-1][self.key] = x
    ...
    def value(self):
        return (
            ContextVar.ctx_stack[-1][self.key]
            if self.key in ContextVar.ctx_stack[-1]
