...
class TinyJit:
    def __init__(self, fxn: Callable):
        ...
        self.jit_cache: List[
            Tuple[Callable, Any]
        ] = []  # TODO: Any should be List[RawBuffer], but this fails
        self.ret: Any = None
        self.input_replace: Dict[
            Tuple[int, int], Tuple[Union[int, str], int, DType]
        ] = (
            {}
    ...
    def __call__(self, *args, **kwargs) -> Any:
        ...
        input_rawbuffers: Dict[Union[int, str], RawBuffer] = {
            cast(Union[int, str], k): cast(RawBuffer, v.realize().lazydata.realized)
        ...
        if self.cnt >= 2:
            for (j, i), (
                ...
                assert (
                    input_rawbuffers[input_name].size == expected_size
