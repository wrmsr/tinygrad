...
def get_buffers(op: LazyOp) -> List[Any]:
    return functools.reduce(
        operator.add,
        [get_buffers(x) if isinstance(x, LazyOp) else [x] for x in op.src],
        [],
    )
...
def get_lazyops(op: LazyOp) -> List[LazyOp]:
    return functools.reduce(
        operator.add, [get_lazyops(x) for x in op.src if isinstance(x, LazyOp)], [op]
    )
...
def map_buffers(real_srcs: Dict[Any, Any], x: Any) -> LazyOp:
    if len(real_srcs) and x in real_srcs:
        return (
            map_buffers(real_srcs, real_srcs[x])
    ...
    return LazyOp(
        x.op,
        tuple(
            (map_buffers(real_srcs, y) if isinstance(y, LazyOp) else real_srcs[y])
            for y in x.src
        ),
        x.arg,
    )
...
class Interpreted:
    ...
    def exec_ast(self, ast: LazyOp, output=None, context=None):
        if (
            FusedOps.MULACC in self.fxn_for_op
        ...
        if not created_context and ast in context:
            return context[ast]
        srcs = [
            self.exec_ast(x, context=context)
        ...
        if DEBUG >= 3:
            print(
                ...
                list(set(x._buf.shape for x in srcs)),
                ast.arg if ast.arg is not None else "",
            )
        if not created_context:
            context[ast] = ret
        if output is not None and output.output_buffer is not None:
            assert output.output_buffer.size == ret.size, (
                output.output_buffer.dtype == ret.dtype
            )
...
class ASTRunner:
    ...
    def exec(self, bufs) -> Optional[float]:
        rawbufs = [
            x.realized
            for x in bufs
            if x.realized is not None and not isinstance(x.realized, RawConst)
        ]
    ...
    def __call__(
        self, rawbufs: List[RawBuffer], jit=False, force_wait=False
    ) -> Optional[float]:
        if (
            getenv("OPTLOCAL")
            and self.global_size is not None
            and self.local_size is None
        ):
            self.local_size = self.optimize_local_size(
                rawbufs, allow_cache=(getenv("OPTLOCAL") >= 2)
            )
        if et := self.clprg(
            self.global_size, self.local_size, *rawbufs, wait=force_wait or DEBUG >= 2
        ):
        ...
        if DEBUG >= 2:
            print(
                ...
                + (
                    str()
                    if et is None
                    else f"tm {et * 1e6:9.2f}us/{GlobalCounters.time_sum_s * 1e3:9.2f}ms ({self.op_estimate / (et * 1e9):8.2f} GFLOPS, {self.mem_estimate / (et * 1e9):7.2f} GB/s)"
                )
            )
        GlobalCounters.kernel_count += 1
        GlobalCounters.global_ops += self.op_estimate
        GlobalCounters.global_mem += self.mem_estimate
        if getenv("EARLY_STOPPING") and GlobalCounters.kernel_count == getenv(
            "EARLY_STOPPING"
        ):
            exit(0)
        return et
    ...
    def optimize_local_size(
        self, rawbufs: List[RawBuffer], preserve_output=False, allow_cache=False
    ) -> List[int]:
        assert (
            self.global_size is not None
        ), "needs a global size to optimize local size"
        if allow_cache:
            import dbm, pickle
            ...
            if self.prg not in ASTRunner.optlocal_cache:
                ASTRunner.optlocal_cache[self.prg] = pickle.dumps(
                    self.optimize_local_size(
                        rawbufs, preserve_output, allow_cache=False
                    )
        ...
        if preserve_output or any(
            x == rawbufs[0] for x in rawbufs[1:]
        ):  # this is an assignment, replace the output buffer
        ...
        MAX_WORKGROUP = (
            self.clprg.max_work_group_size()
...
class Compiled:
    def __init__(
        self, buffer: Type[RawBuffer], codegen, runtime, synchronize=lambda: None
    ):
        self.buffer, self.codegen, self.runtime, self.synchronize = (
            buffer,
            codegen,
            runtime,
            synchronize,
    ...
    def exec_ast(self, ast: LazyOp, output, **kwargs):
        ...
        if (
            ast.op in MovementOps
            and not isinstance(ast.src[0], LazyOp)
            and ast.src[0].realized is not None
        ):
