...
class LazyOp(ta.NamedTuple):
    op: Op
    # ta.Any == ta.Union[LazyOp, LazyBuffer, DeviceBuffer]
    src: ta.Tuple[ta.Any, ...]  # type: ignore
    arg: ta.Any = None
    # TODO: add dest to support multiple outputs. on second thought, multiple outputs will have multiple LazyOps.


# ta.Any == ta.Union[LazyBuffer, DeviceBuffer]
def get_buffers(op: LazyOp) -> ta.List[ta.Any]:
    return functools.reduce(operator.add, [get_buffers(x) if isinstance(x, LazyOp) else [x] for x in op.src], [])


def get_lazyops(op: LazyOp) -> ta.List[LazyOp]:
...
class Interpreted:
    def __init__(
        self,
        buffer,
        fxn_for_op: ta.Dict[Op, ta.Callable],
        from_lazybuffer=lambda x: x.realized,
    ...
    def exec_ast(self, ast: LazyOp, output=None, context=None):
        ...
        srcs = [
            self.exec_ast(x, context=context) if isinstance(x, LazyOp) else self.from_lazybuffer(x)
            for x in ast.src
        ]
...
class FlopCounter:
    def __init__(self, tup: ta.Tuple[ta.Tuple[int, ...], DType, int]) -> None:
...
shape_fxn_for_op: ta.Dict[Op, ta.Callable] = {
    ...
    **{
        ...
        for op in UnaryOps if op != UnaryOps.CAST
    },
    **{
        op: lambda self, y: (
            self.shape,
    ...
    **{
        op: functools.partial(
            lambda mop, self, arg: (
                ShapeTracker(self.shape).movement_op(mop, arg).shape,
                self.dtype,
                self.consume_flops(),
            ),
            op,
...
InterpretedFlopCounter = Interpreted(
    FlopCounter,
    shape_fxn_for_op,
...
class ASTRunner:
    def __init__(
        ...
        name,
        prg,
        global_size: ta.Optional[ta.List[int]] = None,
        local_size: ta.Optional[ta.List[int]] = None,
        op_estimate=0,
        mem_estimate=0,
    ...
    def exec(self, bufs) -> ta.Optional[float]:
        rawbufs = [
            x.realized
            for x in bufs
            if x.realized is not None
               and not isinstance(x.realized, RawConst)
        ]
    ...
    def __call__(self, rawbufs: ta.List[RawBuffer], jit=False, force_wait=False) -> ta.Optional[float]:
        ...
        if DEBUG >= 2:
            print(
                ...
                (
                    ...
                    f"({self.op_estimate / (et * 1e9):8.2f} GFLOPS, "
                    f"{self.mem_estimate / (et * 1e9):7.2f} GB/s)"
                )
            )
    ...
    def timeit(self, rawbufs: ta.List[RawBuffer], local_override=None) -> float:
        try:
            return self.clprg(
                self.global_size,
                local_override if local_override is not None else self.local_size,
                *rawbufs,
                wait=True,
            )
        except Exception:
    ...
    optlocal_cache: ta.ClassVar[ta.Any] = None

    def optimize_local_size(
        self,
        rawbufs: ta.List[RawBuffer],
        ...
        if allow_cache:
            ...
            if self.prg not in ASTRunner.optlocal_cache:
                ASTRunner.optlocal_cache[self.prg] = pickle.dumps(
                    self.optimize_local_size(
                        rawbufs,
                        preserve_output,
                        allow_cache=False,
                    ),
        ...
        local_sizes = ([
            list(x)
            for x in itertools.product(*local_dims) if prod(x) <= MAX_WORKGROUP
        ]) * 2  # try each valid size twice

        return min([
            (self.timeit(rawbufs, local_size), local_size)
...
class Compiled:
    def __init__(
        ...
        self.method_cache: ta.Dict[str, ASTRunner] = {}
