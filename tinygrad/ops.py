...
class UnaryOps(Enum):
    NOOP = auto(); EXP = auto(); LOG = auto(); CAST = auto(); SIN = auto()  # noqa: E702


class BinaryOps(Enum):
    ADD = auto(); SUB = auto(); MUL = auto(); DIV = auto(); POW = auto(); CMPEQ = auto(); MAX = auto()  # noqa: E702
...
class LoadOps(Enum):
    FROMCPU = auto(); CONTIGUOUS = auto(); TOCPU = auto(); CUSTOM = auto()  # noqa: E702
...
def get_buffers(op: LazyOp) -> List[Any]:
    return functools.reduce(operator.add, [get_buffers(x) if isinstance(x, LazyOp) else [x] for x in op.src], [])


def get_lazyops(op: LazyOp) -> List[LazyOp]:
    return functools.reduce(operator.add, [get_lazyops(x) for x in op.src if isinstance(x, LazyOp)], [op])
...
def map_buffers(real_srcs: Dict[Any, Any], x: Any) -> LazyOp:
    ...
    return LazyOp(x.op, tuple((map_buffers(real_srcs, y) if isinstance(y, LazyOp) else real_srcs[y]) for y in x.src),
                  x.arg)
...
class Interpreted:
    def __init__(self, buffer, fxn_for_op: Dict[Op, Callable], from_lazybuffer=lambda x: x.realized,
    ...
    def exec_ast(self, ast: LazyOp, output=None, context=None):
        if FusedOps.MULACC in self.fxn_for_op and ast.op == ReduceOps.SUM and isinstance(ast.src[0], LazyOp) and \
        ...
        if not created_context and ast in context:
            return context[ast]
        srcs = [self.exec_ast(x, context=context) if isinstance(x, LazyOp) else self.from_lazybuffer(x) for x in
                ast.src]
        if DEBUG >= 3:
            st = time.perf_counter()
        ret = self.buffer(self.fxn_for_op[ast.op](
            *([self.to_underlying(x) for x in srcs] + ([ast.arg] if ast.arg is not None else []))))
        if DEBUG >= 3:
            print(
                f"*** {'exec' if created_context else '    '} {GlobalCounters.mem_used / 1e9:5.2f} GB {(time.perf_counter() - st) * 1e3:7.2f} ms op: {ast.op:20s} out({ret.dtype.name}): {str(ret._buf.shape):30s} in({len(srcs)}):",
                list(set(x._buf.shape for x in srcs)), ast.arg if ast.arg is not None else "")
...
class FlopCounter:
    def __init__(self,
                 tup: Tuple[Tuple[int, ...], DType, int]): self.shape, self.dtype, self.flops, self._buf = *tup, self
...
shape_fxn_for_op: Dict[Op, Callable] = {
    ...
    **{op: lambda self: (self.shape, self.dtype, self.consume_flops() + prod(self.shape)) for op in UnaryOps if
       op != UnaryOps.CAST},
    **{op: lambda self, y: (
    self.shape, max(self.dtype, y.dtype), self.consume_flops() + y.consume_flops() + prod(self.shape)) for op in
       BinaryOps},
    **{op: lambda self, new_shape: (new_shape, self.dtype, self.consume_flops() + prod(self.shape)) for op in
       ReduceOps},
    **{op: functools.partial(
        lambda mop, self, arg: (ShapeTracker(self.shape).movement_op(mop, arg).shape, self.dtype, self.consume_flops()),
        op) for op in MovementOps}}
InterpretedFlopCounter = Interpreted(FlopCounter, shape_fxn_for_op, lambda x: FlopCounter((x.shape, x.dtype, 0)),
                                     lambda x: x)
...
class ASTRunner:
    def __init__(self, name, prg, global_size: Optional[List[int]] = None, local_size: Optional[List[int]] = None,
                 op_estimate=0, mem_estimate=0, display_name: Optional[str] = None):
        if DEBUG >= 4:
            print(prg)
    ...
    def __call__(self, rawbufs: List[RawBuffer], jit=False, force_wait=False) -> Optional[float]:
        if getenv("OPTLOCAL") and self.global_size is not None and self.local_size is None:
            self.local_size = self.optimize_local_size(rawbufs, allow_cache=(getenv("OPTLOCAL") >= 2))
        if et := self.clprg(self.global_size, self.local_size, *rawbufs, wait=force_wait or DEBUG >= 2):
            GlobalCounters.time_sum_s += et
        if DEBUG >= 2:
            print(
                f"{colored(f'*** {GlobalCounters.kernel_count:4d}', 'magenta' if jit else None)} {(self.display_name + ' ' * (29 - len(self.name))) if self.display_name is not None else self.name:26s} arg {len(rawbufs):3d} sz {str(self.global_size):18s} {str(self.local_size):12s} OPs {int(self.op_estimate / 1e6):6d}M/{GlobalCounters.global_ops / 1e9:7.2f}G  mem {GlobalCounters.mem_used / 1e9:5.2f} GB " +
                (
    ...
    def timeit(self, rawbufs: List[RawBuffer], local_override=None) -> float:
        try:
            return self.clprg(self.global_size, local_override if local_override is not None else self.local_size,
                              *rawbufs, wait=True)
        except Exception:
            return float('inf')
    ...
    def optimize_local_size(self, rawbufs: List[RawBuffer], preserve_output=False, allow_cache=False) -> List[int]:
        ...
        if allow_cache:
            import dbm, pickle
            if ASTRunner.optlocal_cache is None:
                ASTRunner.optlocal_cache = dbm.open('/tmp/optlocal.db', 'c')
            if self.prg not in ASTRunner.optlocal_cache:
                ASTRunner.optlocal_cache[self.prg] = pickle.dumps(self.optimize_local_size(rawbufs, preserve_output,
                                                                                           allow_cache=False))  # pylint: disable=unsupported-membership-test,unsupported-assignment-operation
            return pickle.loads(ASTRunner.optlocal_cache[self.prg])
        if preserve_output or any(
            x == rawbufs[0] for x in rawbufs[1:]):  # this is an assignment, replace the output buffer
        ...
        local_dims = [[x for x in set([sz, 1, 2, 4, 8, 16, 32, 64, 128, 256, MAX_WORKGROUP]) if x <= sz] for sz in
                      self.global_size]
        local_sizes = [list(x) for x in itertools.product(*local_dims) if
                       prod(x) <= MAX_WORKGROUP] * 2  # try each valid size twice
        return min([(self.timeit(rawbufs, local_size), local_size) for local_size in
                    random.sample(local_sizes, len(local_sizes))])[1]
...
class Compiled:
    ...
    def exec_ast(self, ast: LazyOp, output, **kwargs):
        ...
        if output.realized is not None:
            if isinstance(output.realized, RawConst):
                output.realized = None  # can't assign to RawConst
