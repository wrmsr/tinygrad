...
class Function:
    def __init__(self, device: str, *tensors: Tensor):
        ...
        self.requires_grad = (
            ...
            else (None if any(x is None for x in self.needs_input_grad) else False)
        )
...
class Tensor:
    ...
    def __init__(
        ...
        if isinstance(data, list):
            data = np.array(
                data, dtype=(dtype if dtype is not None else Tensor.default_type).np
            )
        ...
        if isinstance(data, LazyNumpyArray):
            ...
            lazydata = LazyBuffer.fromCPU(
                data.astype(dtype.np) if dtype is not None else data, device
            )
    ...
    def zeros(*shape, **kwargs):
        return Tensor.full(shape, 0, **kwargs)

    @staticmethod
    def ones(*shape, **kwargs):
        return Tensor.full(shape, 1, **kwargs)
    ...
    def empty(*shape, device=Device.DEFAULT, dtype: Optional[DType] = None, **kwargs):
        ...
        return Tensor(
            LazyBuffer.empty(
                [prod(shape)], Tensor.default_type if dtype is None else dtype, device
            ),
            dtype=dtype,
            device=device,
            **kwargs,
    ...
    def randn(*shape, **kwargs) -> Tensor:
        return Tensor(
            LazyNumpyArray(
                lambda lna: Tensor._rng.standard_normal(
                    size=lna.shape, dtype=lna.dtype
                ),
                shape,
                np.float32,
            ),
    ...
    def kaiming_uniform(*shape, a: float = 0.01, **kwargs) -> Tensor:
        bound = (
            math.sqrt(3.0)
            * math.sqrt(2.0 / (1 + a**2))
            / math.sqrt(shape[1] * prod(shape[2:]))
        )
    ...
    def reshape(self, shape, *args) -> Tensor:
        ...
        assert len(new_shape) > 0 and all(
    ...
    def _pool(
        ...
        if any(k > s for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
            ...
            xup = (
                ...
                .expand(
                    *prefix, *_insert_dims, *flatten((e, i) for e, i in zip(e_, i_))
                )
            ...
            if len(_insert_dims):
                ...
                slc_prefix += [(0, x) for x in _insert_dims]
            xup = xup.slice(
                slc_prefix + flatten(((0, o), (0, k)) for o, k in zip(o_, k_))
            )
            return xup.permute(
                *range(len(prefix)),
                *[len(prefix) + i * 2 for i in range(len(k_))],
    ...
    def conv_transpose2d(
        ...
        padding = flatten(
            ((k - 1) * d - p, (k - 1) * d - p)
            for k, p, d in reversed(
                list(zip(HW, make_pair(padding, len(HW)), make_pair(dilation, len(HW))))
            )
        )
        return x.conv2d(
    ...
    def __ne__(self, x) -> Tensor:
        return 1.0 - self.eq(x)  # type: ignore
    ...
    def dropout(self, p=0.5) -> Tensor:
        ...
        _mask: np.ndarray = np.asarray(
            Tensor._rng.binomial(1, 1.0 - p, size=self.shape), dtype=np.float32
        )
        return (
            self
            * Tensor(_mask, requires_grad=False, device=self.device)
            * (1 / (1.0 - p))
        )
