...
class Tensor:
    ...
    def __repr__(self):
        return (
            f"<Tensor "
            f"{self.lazydata if self.lazydata.realized is None else self.lazydata.realized!r} "
            f"on {self.device} "
            f"with grad {(self.grad.lazydata if self.grad else None)!r}"
            f">"
        )
    ...
    def rand(*shape, **kwargs) -> 'Tensor':
        return Tensor(
            LazyNumpyArray(
                lambda lna: Tensor._rng.random(size=lna.shape, dtype=lna.dtype),
                shape,
                np.float32,
            ),
            **kwargs,
    ...
    def randn(*shape, **kwargs) -> 'Tensor':
        return Tensor(
            LazyNumpyArray(
                lambda lna: Tensor._rng.standard_normal(size=lna.shape, dtype=lna.dtype),
                shape,
                np.float32,
            ),
            **kwargs,
    ...
    def reshape(self, shape, *args) -> 'Tensor':
        ...
        return mlops.Reshape.apply(
            self, shape=tuple(-prod(self.shape) // prod(new_shape) if s == -1 else s for s in new_shape))
    ...
    def __getitem__(self, val):
        def slcfix(i, sz, default):
            ...
            return default if i is None else max(0, min(sz, sz + i if i < 0 else i))
        ...
        for i, (sz, s) in enumerate(zip(self.shape, [v for v in val if v is not None])):
            ...
            new_slice.append(
                (s % sz, s % sz + 1) if isinstance(s, int) else
                (slcfix(s.start, sz, 0), slcfix(s.stop, sz, sz)))

        # Shape depends on slices + positions of Nones
        for s, sz in zip(val, [self.shape[i - 1] for i in itertools.accumulate([int(s is not None) for s in val])]):
    ...
    def _pool(
        ...
        if any(k > s for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
            ...
            xup = (
                self
                .reshape(*prefix, *([1] * len(_insert_dims)), *flatten((1, i) for i in i_))
    ...
    def avg_pool2d(self, kernel_size=(2, 2), stride=None):
        return self._pool(make_pair(kernel_size), stride if stride is not None else kernel_size) \
    ...
    def conv2d(
        ...
        # x = self.pad2d(padding_)._pool((H,W), stride, dilation, _insert_dims=(cout//groups,))   # (bs, groups*cin, rcout, oy, ox, H, W)  # noqa
    ...
    def _broadcasted(
        ...
        x, y = [
            Tensor([t], device=self.device, requires_grad=False)
            if not isinstance(t, Tensor) else t
    ...
    def batchnorm(
        ...
        if len(invstd.shape) == 1:
            ret = x.mul(invstd.reshape(shape=[1, -1, 1, 1]))
        else:
            ret = invstd
