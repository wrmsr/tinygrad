...
class AdamW(Optimizer):
    def __init__(
        self, params: List[Tensor], lr=0.001, b1=0.9, b2=0.999, eps=1e-8, wd=0.01
    ):
        super().__init__(params)
        # NOTE: self.t is a tensor so Adam can be jitted
        self.lr, self.b1, self.b2, self.eps, self.wd, self.t = (
            lr,
            b1,
            b2,
            eps,
    ...
    def step(self) -> None:
        ...
        for i, t in enumerate(self.params):
            ...
            t.assign(
                t.detach()
                - a * self.m[i].div(self.v[i].sqrt() + self.eps)
                - self.lr * self.wd * t.detach()
            )
...
def get_state_dict(obj, prefix: str = "") -> Dict[str, Tensor]:
    ...
    if isinstance(obj, (list, tuple)):
        for i, x in enumerate(obj):
            state_dict.update(get_state_dict(x, f"{prefix}{str(i)}."))
    elif isinstance(obj, dict):
        for k, v in obj.items():
            state_dict.update(get_state_dict(v, f"{prefix}{str(k)}."))
    return state_dict
