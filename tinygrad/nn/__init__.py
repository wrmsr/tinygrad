...
class Conv2d:
    def __init__(
        ...
        self.weight = getattr(Tensor, initialization)(
...
class GroupNorm:
    ...
    def __call__(self, x: Tensor):
        ...
        return x * self.weight.reshape(
            1, -1, *[1 for _ in range(len(x.shape) - 2)]
...
class Embedding:
    ...
    def __call__(self, idx: Tensor) -> Tensor:
        vocab_counter = (
            Tensor.arange(self.vocab_size, requires_grad=False)
            .reshape(1, 1, self.vocab_size)
            .expand(*idx.shape, self.vocab_size)
        )
        return (
            vocab_counter == idx.unsqueeze(2).expand(*idx.shape, self.vocab_size)
        ) @ self.weight
