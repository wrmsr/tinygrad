...
def stfetch_mnist():
    ...
    X_train = (
        parse(os.path.dirname(__file__) + "/mnist/train-images-idx3-ubyte.gz")[0x10:]
        .reshape((-1, 28 * 28))
        .astype(np.float32)
    )
    Y_train = parse(os.path.dirname(__file__) + "/mnist/train-labels-idx1-ubyte.gz")[8:]
    X_test = (
        parse(os.path.dirname(__file__) + "/mnist/t10k-images-idx3-ubyte.gz")[0x10:]
        .reshape((-1, 28 * 28))
        .astype(np.float32)
    )
...
def fetch_cifar(train=True):
    cifar10_mean = np.array(
        [0.4913997551666284, 0.48215855929893703, 0.4465309133731618], dtype=np.float32
    ).reshape(1, 3, 1, 1)
    cifar10_std = np.array(
        [0.24703225141799082, 0.24348516474564, 0.26158783926049628], dtype=np.float32
    ).reshape(1, 3, 1, 1)
    ...
    if train:
        db = [
            pickle.load(
                tt.extractfile(f"cifar-10-batches-py/data_batch_{i}"), encoding="bytes"
            )
            for i in range(1, 6)
        ]
    else:
