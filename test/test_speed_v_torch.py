...
class TestSpeed(unittest.TestCase):
    ...
    def test_openpilot_conv2d(self):
        ...
        torch_conv = torch.nn.Conv2d(in_chans, out_chans, 3, bias=None, padding=1).to(
            torch_device
        )
        ...
        helper_test_generic(
            f"conv bs:{bs:3d} chans:{in_chans:3d} -> {out_chans:3d}",
            f1,
            (torch_dat,),
            TinyJit(f2),
            (tiny_dat,),
    ...
    def test_conv2d(self):
        ...
        for bs in [32]:
            for in_chans in IN_CHANS:
                for out_chans in [32]:
                    img_size = 34
                    torch_dat = torch.rand(bs, in_chans, img_size, img_size).to(
                        torch_device
                    )
                    torch_conv = torch.nn.Conv2d(in_chans, out_chans, 3, bias=None).to(
                        torch_device
                    )
