...
def helper_test_speed(f1, *args):
    ...
    for i in range(CNT):
        ...
        # force syncing
        [x.numpy() if isinstance(x, Tensor) or str(torch_device) == "cpu" else x.cpu().numpy() for x in args if
         x is not None]
        ...
        if i >= 1:
            ets.append(et)  # not the first run / one used for OPTLOCAL
...
def helper_test_generic_square(name, N, f1, f2, onearg=False):
    ...
    helper_test_generic(f"{name:30s} {N:4d}x{N:4d}", f1, (torch_a, torch_b), TinyJit(lambda a, b: f2(a, b).realize()),
                        (tiny_a, tiny_b))
...
def helper_test_generic(name, f1, f1_args, f2, f2_args):
    ...
    mem = save_mem * 1e-6
    print(
        f"{prefix}{name:40s} {et_torch:7.2f} ms ({flops / et_torch:8.2f} GFLOPS {mem / et_torch:8.2f} GB/s) in torch, {et_tinygrad:7.2f} ms ({flops / et_tinygrad:8.2f} GFLOPS {mem / et_tinygrad:8.2f} GB/s) in tinygrad, {colorize_float(et_tinygrad / et_torch)} {desc} {flops:7.2f} MOPS {mem:7.2f} MB")
    prefix = " "
...
class TestSpeed(unittest.TestCase):
    ...
    def test_double_permute(self):
        ...
        helper_test_generic(f"double_permute {tiny_a.shape}", f, (torch_a,), TinyJit(lambda a: f(a).realize()),
                            (tiny_a,))
    ...
    def test_gemm_unrolled_permute_l(self):
        ...
        def f2(a, b): return (
                a.permute(1, 0).reshape(N, 1, N).expand(N, N, N) * b.reshape(1, N, N).expand(N, N, N)).sum(
            axis=2)
    ...
    def test_gemm_unrolled_permute_r(self):
        ...
        def f2(a, b): return (
                a.reshape(N, 1, N).expand(N, N, N) * b.permute(1, 0).reshape(1, N, N).expand(N, N, N)).sum(
            axis=2)
    ...
    def test_gemm_unrolled_permute_lr(self):
        ...
        def f2(a, b): return (
            a.permute(1, 0).reshape(N, 1, N).expand(N, N, N) * b.permute(1, 0).reshape(1, N, N).expand(N, N, N)).sum(
            axis=2)
    ...
    def test_openpilot_conv2d(self):
        ...
        helper_test_generic(f"conv bs:{bs:3d} chans:{in_chans:3d} -> {out_chans:3d}", f1, (torch_dat,), TinyJit(f2),
                            (tiny_dat,))
        ...
        for bs in [32]:
            for in_chans in IN_CHANS:
                for out_chans in [32]:
                    ...
                    helper_test_generic(f"conv bs:{bs:3d} chans:{in_chans:3d} -> {out_chans:3d}", f1, (torch_dat,),
                                        TinyJit(f2),
                                        (tiny_dat,))
