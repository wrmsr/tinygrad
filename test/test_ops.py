...
def helper_test_op(
    ...
    if shps is None:
        ...
        ts = [
            torch.tensor(
                (np.random.random(size=x).astype(np.float32) + a) * b,
                requires_grad=True,
            )
            for x in shps
        ]
...
class TestOps(unittest.TestCase):
    ...
    def test_std_axis(self):
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, unbiased=False, dim=0),
            lambda x: Tensor.std(x, axis=0),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, unbiased=False, dim=2),
            lambda x: Tensor.std(x, axis=2),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, unbiased=False, dim=[1, 2]),
            lambda x: Tensor.std(x, axis=[1, 2]),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, unbiased=False, dim=None),
    ...
    def test_std_keepdim(self):
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, keepdim=True),
    ...
    def test_slice(self):
        ...
        helper_test_op(
            [(3, 3, 3, 3)], lambda x: x[1:2, 1:2, 0:-1], lambda x: x[1:2, 1:2, 0:-1]
        )
    ...
    def test_dilated_conv_transpose2d(self):
        helper_test_op(
            ...
            lambda x, w: Tensor.conv_transpose2d(x, w, dilation=2).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
        )
    ...
    def test_strided_conv_transpose2d(self):
        helper_test_op(
            ...
            lambda x, w: Tensor.conv_transpose2d(x, w, stride=2).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
        )
    ...
    def test_cat(self):
        for dim in range(-1, 2):
            helper_test_op(
                [(45, 65), (45, 65)],
                lambda x, y: torch.cat((x, y), dim),
                lambda x, y: x.cat(y, dim=dim),
            )
