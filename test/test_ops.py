...
def helper_test_op(shps, torch_fxn, tinygrad_fxn=None, atol=1e-6, rtol=1e-3, grad_atol=1e-4, grad_rtol=1e-3,
    ...
    if not forward_only and not FORWARD_ONLY:
        ...
        for i, (t, tt) in enumerate(zip(ts, tst)):
            compare(f"backward pass tensor {i}", tt.grad.numpy(), t.grad.detach().numpy(), atol=grad_atol,
                    rtol=grad_rtol)
...
class TestOps(unittest.TestCase):
    ...
    def _test_cmp(self, fxn, reverse=True):
        ...
        helper_test_op(None, lambda x, y: fxn(x, 2), lambda x, y: fxn(x, 2), forward_only=True,
                       vals=[[0., 1, 2], [2., 1, 0]])
        if reverse:
            helper_test_op(None, lambda x, y: fxn(2, y), lambda x, y: fxn(2, y), forward_only=True,
                           vals=[[0., 1, 2], [2., 1, 0]])
    ...
    def test_relu_maximum_exact(self):
        helper_test_op(None, lambda x: torch.maximum(x, torch.zeros_like(x, requires_grad=False)),
                       lambda x: Tensor.maximum(x, 0), vals=[[-1., 0, 1]])
    ...
    def test_std_axis(self):
        ...
        helper_test_op([(45, 65, 85)], lambda x: torch.std(x, unbiased=False, dim=[1, 2]),
                       lambda x: Tensor.std(x, axis=[1, 2]))
        helper_test_op([(45, 65, 85)], lambda x: torch.std(x, unbiased=False, dim=None),
    ...
    def test_log_softmax(self):
        helper_test_op([(45, 65)], lambda x: torch.nn.LogSoftmax(dim=1)(x), Tensor.log_softmax, atol=1e-7,
                       grad_atol=1e-7)

    def test_log_softmax_other_axis(self):
        helper_test_op([(10, 10, 10)], lambda x: x.log_softmax(0), lambda x: x.log_softmax(0), atol=1e-7,
                       grad_atol=1e-7)
        helper_test_op([(10, 10, 10)], lambda x: x.log_softmax(1), lambda x: x.log_softmax(1), atol=1e-7,
                       grad_atol=1e-7)
        helper_test_op([(10, 10, 10)], lambda x: x.log_softmax(2), lambda x: x.log_softmax(2), atol=1e-7,
                       grad_atol=1e-7)
    ...
    def test_hardtanh(self):
        for val in range(10, 30, 5):
            helper_test_op([(45, 65)], lambda x: torch.nn.functional.hardtanh(x, -val, val),
                           lambda x: x.hardtanh(-val, val), atol=1e-6, grad_atol=1e-6)
    ...
    def test_transpose(self):
        ...
        helper_test_op([(1, 2, 3, 4)], lambda x: x.movedim((3, 0, 2, 1), (0, 1, 2, 3)),
    ...
    def test_flip(self):
        ...
        helper_test_op([(4, 3, 6, 6)], lambda x: torch.flip(x, (0, 1, 3)).flip((0,)),
    ...
    def test_nested_conv2d(self):
        helper_test_op([(1, 32, 9, 9), (32, 32, 3, 3), (32, 32, 3, 3)],
                       lambda x, w1, w2: torch.nn.functional.conv2d(torch.nn.functional.conv2d(x, w1).relu(),
                                                                    w2).relu(),
    ...
    def test_simple_conv2d_nhwc(self):
        ...
        helper_test_op([(2, 9, 9, 10), (3, 3, 10, 20)],
                       ...
                       lambda x, w: Tensor.conv2d(x.permute(0, 3, 1, 2), w.permute(3, 2, 0, 1)).relu(), atol=1e-4,
                       grad_rtol=1e-5)
    ...
    def test_multicat(self):
        for dim in range(-1, 2):
            helper_test_op([(45, 65), (45, 65), (45, 65)], lambda x, y, z: torch.cat((x, y, z), dim),
    ...
    def test_stack(self):
        ...
        for dim in range(-1, 3):
            helper_test_op([(45, 65, 3), (45, 65, 3), (45, 65, 3)], lambda x, y, z: torch.stack((x, y, z), dim=dim),
