        ...
        for i, arg in enumerate(args):
            ...
            if key not in bufs:
                if key in special_names:
                    ...
                    bufnum += 1
                    if i > 0:
                        bufs_to_save[bufs[key][
                            0]] = arg  # if first usage of a buffer is not an output, and it's not a special name
...
if __name__ == "__main__":
    ...
    @TinyJit
    def run(x):
        return model.forward(x).realize()
    ...
    # image library!
    cprog += ["#define STB_IMAGE_IMPLEMENTATION",
              fetch("https://raw.githubusercontent.com/nothings/stb/master/stb_image.h").decode('utf-8')]
    ...
    cprog += [f"float {name}[{len}];" if name not in bufs_to_save else f"float *{name} = (float *){name}_data;" for
              name, len in bufs.values()]
