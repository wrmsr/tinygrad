...
from extra.utils import fake_torch_load
...
class BasicModule:
    def __init__(self, c0, c1, c2):
        self.basic_module = {
            "SingleConv1": SingleConv(c0, c1),
            "SingleConv2": SingleConv(c1, c2),
        }
...
class UNet3D:
    def __init__(self):
        ...
        self.encoders = [
            BasicModule(ups[i] if i != 0 else 1, ups[i], ups[i + 1]) for i in range(4)
        ]
        self.decoders = [
            BasicModule(ups[-1 - i] + ups[-2 - i], ups[-2 - i], ups[-2 - i])
            for i in range(3)
        ]
    ...
    def __call__(self, x):
        ...
        for e in self.encoders:
            intermediates.append(e(intermediates[-1]))
        ret = intermediates[-1]
        for d, i in zip(self.decoders, intermediates[:-1][::-1]):
            ret = d(ret.cat(i, dim=1))
        return ret

    def load_from_pretrained(self):
        fn = Path(__file__).parent.parent / "weights/unet-3d.ckpt"
        download_file(
            "https://oc.embl.de/index.php/s/61s67Mg5VQy7dh9/download?path=%2FLateral-Root-Primordia%2Funet_bce_dice_ds1x&files=best_checkpoint.pytorch",
            fn,
        )
