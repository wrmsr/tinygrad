...
class TransformerBlock:
    def __init__(
        self, embed_dim, num_heads, ff_dim, prenorm=False, act=lambda x: x.relu()
    ):
    ...
    def attn(self, x):
        ...
        query, key, value = [
            x.linear(*y).reshape(shape=(x.shape[0], -1, self.num_heads, self.head_size))
        ...
        attention = weights.dot(value).permute(
            order=(0, 2, 1, 3)
